...
---
## Task 70 — DraftPro polish round 3: draft-all pre-drafting, overlap-proof pagination, highlight placeholders, gap-free drafts (commit eef021fe)

**User feedback (2026-09-24):** placeholder colors illegible (want a highlight, not re-dyed text); text overlapping in the editor ("never seen it in any other word processor"); "Draft all" left documents #2..n undrafted — clicking one visibly started drafting from scratch (wanted start/complete cues in the chat + instant opens); persistent "weird gaps" / unprofessional formatting (proper Nigerian court-process headings, professional line spacing — "sort this out once and for all"); learn from web research in terms of law/drafting but NEVER from user data; general ALOA→DraftPro polish.

**Fixes shipped:**

1. **Placeholder highlights (LegalPlaceholder.tsx)** — category color is now a translucent background band (highlighter pen: `bg-blue-200/70` etc.) + a small colour dot + the dashed underline; the text keeps the editor's own ink (#111827) instead of being re-dyed `text-blue-900`. Also fixed: court reference numbers (`[SUIT NUMBER]`, `[CASE NO]`) previously fell into the duration bucket ("NUMBER") — both resolvers (editor + draftHtml) now check court-reference patterns first.

2. **AutoPagination REWRITE (DraftProEditor)** — the old running-total accounting ignored margin-top, CSS margin collapse, and blocks taller than a page; drift accumulated until content rendered on top of the next page's text (the overlap report). New position-based pass: measures each top-level block's real `offsetTop`/`offsetHeight`, strips the previously-applied jump margins to recover natural gaps, then simulates page assignment in flow space. Oversized blocks flow across the boundary (a word processor would split them) — never stacked on following text. Re-measures on zoom change, `document.fonts.ready` and window resize. `pageCount` formula fixed to account for the inter-sheet gap (the old `ceil(contentHeight / PAGE_HEIGHT_PX)` undercounted pages → content ran past the last painted sheet).

3. **"Draft all" now drafts ALL (DocumentPacketCard + AloaChat)** — document #1 opens live in DraftPro (consumes the user gesture); documents #2..n are FULLY drafted in the background by the chat (`preDraftPacketDoc`: 2-worker pool, module-level in-flight dedupe, hadContent guard, `draftHasSubstance` empty-output guard) and persisted through the same draft-session carrier — so opening any of them loads the finished draft instantly (verified: no drafting overlay, correct per-document content). In-chat visual cues: per-row Queued → Drafting… (spinner) → "Drafted — ready to open ✓", header counter ("x/y drafted · n in progress"), a progress bar, and a completion toast. Rows that already have persisted drafts mount as "Drafted" (survives reloads).

4. **draftHtml.ts — one shared deterministic draft pipeline** — replaces the FOUR slightly-different inline regex chains (stream preview / final / abort-recovery / packet): strips code fences, converts markdown bold, converts `<br><br>` to a real paragraph break, deletes every empty/whitespace/`&nbsp;`-only/`<br>`-only paragraph (the "weird gaps"), converts `[PLACEHOLDER]`s to chips (citation markers like [1] excluded), and exposes `draftHasSubstance`. The E2E streams a deliberately messy 7-page draft and proves the editor renders 0 empty paragraphs, 0 `<br>` pairs, 0 literal brackets.

5. **Professional spacing + Nigerian court-process headings** — editor CSS: paragraphs 0.7em after (≈8pt at 12pt body), heading margins rebalanced (generous above, tight below), h3 underline removed, first-paragraph-after-heading tight, lists tightened; line spacing fixed at exactly 1.5 everywhere. DRAFT_QUALITY_BAR §F (19–26): court caption block order (centred, uppercase, one line each), `SUIT NO:` left-aligned below the caption, parties block with numbered designations, process title centred/bold/uppercase (never underlined), numbered facts with real list semantics, right-aligned counsel signature block, and a "ZERO GAPS" self-check rule ("a draft with a stray empty paragraph or <br><br> anywhere is a FAILED draft").

6. **legalPlaybookStore.ts — learn from the web, never from user data** — planning a packet persists the researched process/legal/document knowledge as a firm-scoped playbook (localStorage, LRU 20, merge-on-≥60%-title-overlap) with a defensive scrubber (emails, phone numbers, long digit runs, bracketed personal facts). Playbooks are injected into every packet draft AND regular `start_drafting` prompt as "FIRM RESEARCH KNOWLEDGE (NOT user facts)". E2E-verified: the user's email never lands in the store; a similar future job retrieves the playbook with score 1.00.

7. **"Why this matters" floater ambush fix (GettingStartedChecklist, caught by this task's E2E)** — the tips floater anchors bottom-right (`sm:left-auto sm:w-[350px]`), exactly over the open ALOA/ARIA chat panel, and swallowed clicks on packet action buttons. It no longer renders while the assistant panel is open (same suppression pattern as the 2026-09-23 tour fix).

**Verification:** E2E `scripts/aloa-task70-verify.cjs` — 29/29 PASS against the local Convex backend + mocked Gemini: fresh Vega signup (ToS/Privacy checkboxes → sqlite verification-code retrieval → 6-step wizard on a Growth trial → AI consent), packet plan, Draft-all (live tab + 2 background drafts), per-document content markers, gap/bracket audits, 187/187 + 39/39 placeholder-highlight + ink checks, 54-block and 17-block no-overlap audits, playbook save/retrieve/no-user-data checks. Screenshots VLM-inspected (`audit-shots/task70/`). Gates: tsc 129 (baseline), vitest 1181/1181 (+31 new), UI ratchet 2372 held, eslint 0 errors, vite build green. Commit eef021fe pushed to main (rebased onto the 1.0.650 version bump).
