                        ...
                        <div className="flex-1 min-w-0 pr-2">
                            ...
                            <div className="flex items-center gap-2 mt-0.5">
                                {unitCount && unitCount > 1 && (
                                    <span className="px-1.5 py-0.5 text-3xs font-bold uppercase rounded bg-slate-100 dark:bg-zinc-700 text-slate-600 dark:text-zinc-300">{unitCount} Units</span>
                                )}
                                <span className="text-2xs text-slate-500 dark:text-zinc-400 font-medium truncate max-w-[140px]">{ownerName}</span>
                                {allRents.length > 0 ? (
                                    ...
                                    <span className="text-2xs font-medium text-slate-400 dark:text-zinc-500 italic">Vacant</span>
                                )}
                            </div>
                        </div>
                        <div className="flex items-center gap-1.5 flex-shrink-0">
                            {leaseUrgent && (
                                <span className="text-3xs font-black px-1.5 py-0.5 rounded bg-amber-50 dark:bg-amber-900/20 text-amber-600 dark:text-amber-400 border border-amber-200 dark:border-amber-800">
                                    ⚠ {daysLeft}d
                                </span>
                            )}
                            {leaseExpired && isOccupied && (
                                <span className="text-3xs font-black px-1.5 py-0.5 rounded bg-red-50 dark:bg-red-900/20 text-red-600 dark:text-red-400 border border-red-200 dark:border-red-800">
                                    Expired
                                </span>
                            )}
                            <span className={`text-3xs px-2 py-0.5 rounded-full font-black uppercase tracking-widest ${
                                isOccupied 
            ...
            <div className="flex flex-col h-full bg-slate-50 dark:bg-zinc-900/30 border-r border-slate-200 dark:border-zinc-800">
                ...
                <div className="flex-grow flex flex-col overflow-hidden">
                    ...
                    <div className="bg-slate-50 dark:bg-zinc-900/50 px-4 py-2 border-b border-slate-200 dark:border-zinc-800 flex items-center justify-between">
                        ...
                        <span className="text-2xs font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest">{filteredProperties.length} Total Properties</span>
                    </div>
        ...
        <div className="h-full overflow-y-auto scroll-smooth-ios custom-scrollbar bg-slate-50 dark:bg-zinc-900 pb-nav">
            ...
            <div className="px-4 sm:px-6 lg:px-8">
                    ...
                    <div className="bg-primary-600 px-6 py-3 flex items-center justify-between animate-in slide-in-from-top-4 duration-300 relative z-20">
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex items-center gap-2">
                                <button
                                    onClick={() => openModal('bulkEditProperty', null, { propertyIds: Array.from(selectedIds) })}
                                    className="flex items-center gap-2 px-4 py-1.5 bg-white/10 hover:bg-white/20 text-white rounded-lg text-2xs font-black uppercase tracking-widest transition-all border border-white/10"
                                >
                                    <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                                    Bulk Edit
                                </button>
                                <button
                                    onClick={handleBulkDelete}
                                    className="flex items-center gap-2 px-4 py-1.5 bg-rose-500/20 hover:bg-rose-500 text-white rounded-lg text-2xs font-black uppercase tracking-widest transition-all border border-rose-500/30"
                                >
                ...
                <div className="bg-slate-50 dark:bg-zinc-900/50 px-6 py-3 border-x border-b border-slate-200 dark:border-zinc-700 flex items-center justify-between">
                    ...
                    <div className="text-2xs font-black text-slate-400 dark:text-zinc-500 uppercase tracking-widest">
