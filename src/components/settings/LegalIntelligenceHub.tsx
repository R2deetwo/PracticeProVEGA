...
const StatusBadge: React.FC<{ status: LegalModule['status']; isBundled: boolean }> = ({ status, isBundled }) => {
    if (status === 'active') {
        return (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-2xs font-bold text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/40 border border-emerald-200 dark:border-emerald-800">
    ...
    if (status === 'locked') {
        return (
            <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-2xs font-bold text-amber-700 dark:text-amber-300 bg-amber-50 dark:bg-amber-950/40 border border-amber-200 dark:border-amber-800">
    ...
    return (
        <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-2xs font-bold text-slate-500 dark:text-zinc-400 bg-slate-100 dark:bg-zinc-800 border border-slate-200 dark:border-zinc-700">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-start justify-between gap-2 flex-wrap">
                        <div className="min-w-0">
                            ...
                            <p className="text-2xs text-slate-500 dark:text-zinc-400 mt-0.5 leading-tight truncate">
                                {module.name}
                            </p>
                        </div>
                    ...
                    <div className="flex items-center gap-3 mt-2 flex-wrap">
                        <span className="text-2xs text-slate-400 dark:text-zinc-500">{module.jurisdiction}</span>
                        <span className="text-2xs text-slate-300 dark:text-zinc-700">·</span>
                        <span className="text-2xs text-slate-400 dark:text-zinc-500">{module.version}</span>
                        {module.authority && (
                            <>
                                <span className="text-2xs text-slate-300 dark:text-zinc-700">·</span>
                                <span className="text-2xs text-slate-400 dark:text-zinc-500 italic">{module.authority}</span>
                            </>
                        )}
                    </div>
                </div>
            ...
            {expanded && (
                <div className="px-4 pb-4 pt-0 space-y-3 border-t border-slate-100 dark:border-zinc-700/50">
                    ...
                    {module.coverageAreas.length > 0 && (
                        <div>
                            <p className="text-2xs font-bold text-slate-400 dark:text-zinc-500 uppercase tracking-wider mb-1.5">
                                Covers
                            </p>
                    ...
                    {(module.status === 'locked' || module.status === 'coming_soon') && (
                        <div className="pt-1">
                            <button className="text-2xs font-semibold text-amber-600 dark:text-amber-400 hover:underline">
                    ...
                    <table className="w-full text-left">
                        <thead className="bg-slate-50 dark:bg-zinc-800/80 border-b border-slate-200 dark:border-zinc-700">
                            <tr>
                                ...
                                <th className="py-4 px-6 font-bold text-slate-500 dark:text-zinc-400 text-2xs uppercase tracking-wider text-center">Plan</th>
                                <th className="py-4 px-6 font-bold text-slate-500 dark:text-zinc-400 text-2xs uppercase tracking-wider text-center">Status</th>
                                <th className="py-4 px-6 font-bold text-slate-500 dark:text-zinc-400 text-2xs uppercase tracking-wider text-right">Action</th>
                            </tr>
                        </thead>
                                ...
                                return (
                                    <tr key={l._id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/30 transition-colors text-xs">
                                        ...
                                        <td className="py-4 px-6 text-center">
                                            <span className="px-2 py-1 bg-slate-100 dark:bg-zinc-800 rounded-md font-bold text-3xs uppercase text-slate-600 dark:text-zinc-400">
                                                {l.plan}
                                            </span>
                                        </td>
                                        <td className="py-4 px-6">
                                            <div className="flex justify-center">
                                                {l.isActive ? (
                                                    <span className="inline-flex items-center gap-1 text-emerald-600 dark:text-emerald-400 font-bold uppercase text-3xs">
                                                        <CheckCircleIcon className="w-3 h-3" /> Active
                                                    </span>
                                                ) : (
                                                    <span className="inline-flex items-center gap-1 text-rose-500 font-bold uppercase text-3xs">
            ...
            <div className="overflow-x-auto">
                <table className="w-full text-left">
                    <thead className="bg-slate-50/20 dark:bg-zinc-800/20 border-b border-slate-200 dark:border-zinc-700/50">
                        <tr>
                            <th className="py-4 px-6 font-bold text-slate-500 dark:text-zinc-400 text-2xs uppercase tracking-wider">Time</th>
                            <th className="py-4 px-6 font-bold text-slate-500 dark:text-zinc-400 text-2xs uppercase tracking-wider">Tenant</th>
                            <th className="py-4 px-6 font-bold text-slate-500 dark:text-zinc-400 text-2xs uppercase tracking-wider">Action</th>
                            <th className="py-4 px-6 font-bold text-slate-500 dark:text-zinc-400 text-2xs uppercase tracking-wider">Source Engine</th>
                        </tr>
                    </thead>
                        ...
                        {usageLogs.map((log: any) => {
                            ...
                            return (
                                <tr key={log._id} className="hover:bg-slate-50/50 dark:hover:bg-zinc-800/20 transition-all group">
                                    <td className="py-4 px-6 text-2xs font-mono text-slate-400 transition-colors group-hover:text-slate-500">
                                    ...
                                    <td className="py-4 px-6 text-xs font-bold text-slate-700 dark:text-zinc-300">
                                        <div className="flex items-center gap-2">
                                            <div className="w-6 h-6 rounded bg-slate-100 dark:bg-zinc-800 flex items-center justify-center text-3xs font-black text-slate-400 uppercase">F</div>
            ...
            <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-slate-200 dark:border-zinc-800 pb-6">
                ...
                <div className="flex flex-col items-end gap-3 self-start lg:self-center">
                    <button 
                        ...
                        className={`flex items-center gap-2 px-4 py-1.5 rounded-full text-xs font-bold transition-all border ${showAloaX ? 'bg-indigo-50 border-indigo-200 text-indigo-700 dark:bg-indigo-900/30 dark:border-indigo-800 dark:text-indigo-300' : 'bg-white dark:bg-zinc-900 border-slate-200 text-slate-600 dark:bg-zinc-800 dark:border-zinc-700 dark:text-zinc-400'}`}
                    >
            ...
            <div className="grid grid-cols-3 gap-3">
                {[
                    ...
                    <div key={label} className={`p-4 rounded-lg border ${
                        ...
                        <p className="text-2xs font-medium text-slate-500 dark:text-zinc-400 mt-0.5">{label}</p>
                    </div>
                ))}
            </div>
                        ...
                        <div key={category} className="space-y-3">
                            ...
                            >
                                <div className="flex items-center gap-3">
                                    ...
                                    <span className="text-2xs font-bold text-slate-300 dark:text-zinc-600 px-2 py-0.5 rounded-md border border-slate-100 dark:border-zinc-800">
                                        {modules.length}
                                    </span>
                                </div>
