...
const RoleBadge: React.FC<{ role: UserRole, isPrimaryAdmin: boolean }> = ({ role, isPrimaryAdmin }) => {
    ...
    // Default/Other
        ...
        classes = "bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:bg-zinc-700 dark:text-zinc-300 border border-slate-200 dark:border-zinc-600";
    }
...
const FirmSettings: React.FC<FirmSettingsProps> = ({ firmDetails, onUpdateFirmDetails, openModal, users, currentUser, openUserModal, onDeleteUser, onEnableDevMode }) => {
    ...
    return (
        <div className="space-y-8">
            ...
            {!isProperty && (
                <SettingsCard title="Trust Accounting" id="trust-accounting">
                    <div className="flex items-center justify-between gap-4">
                        ...
                        >
                            <span className={`inline-block h-4 w-4 transform rounded-full bg-white dark:bg-zinc-900 transition-transform ${firmDetails.trustAccountingEnabled ? 'translate-x-6' : 'translate-x-1'}`} />
                        </button>
                    </div>
                </SettingsCard>
            )}
            ...
            {/* Estate Community Features — only for Atrium (property) firms.
                Vega legal firms don't manage physical estates with shared amenities. */}
            {isProperty && <EstateCommunitySettings />}
                                ...
                                <div className="flex-grow w-full sm:w-auto">
                                    <div className="bg-white dark:bg-zinc-900 dark:bg-black/30 border border-slate-200 dark:border-white/10 rounded-lg p-4 flex items-center justify-between">
                                        ...
                                        <button
                                            ...
                                            className="px-3 py-1 bg-slate-100 dark:bg-zinc-800 dark:bg-white/10 hover:bg-slate-200 dark:hover:bg-white/20 rounded-md text-2xs font-bold uppercase transition-colors text-slate-600 dark:text-slate-200"
                                        >
                                            Copy
                                        </button>
                                    </div>
