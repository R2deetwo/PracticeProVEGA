...
const DataManagementSettings: React.FC<DataManagementSettingsProps> = ({ onEnableDevMode }) => {
    ...
    return (
        <div className="space-y-8">
            ...
            {/* ── TASK 16: Content Protection Toggle ── */}
            <SettingsCard>
                    ...
                    <div className="flex items-start justify-between gap-4 mb-4">
                        <div>
                            ...
                            <p className="text-sm text-slate-500 dark:text-zinc-400 leading-relaxed">
                                When enabled, copy/paste, right-click, and screenshots are blocked in the app to protect sensitive data. Turn this off if you need to take screenshots (e.g. for support).
                            </p>
                        </div>
                    </div>
...
const ContentProtectionToggle: React.FC = () => {
    const [enabled, setEnabled] = React.useState(() => {
        try {
            return localStorage.getItem('practicepro_content_protection') !== 'false';
        } catch {
            return true;
        }
    });
    ...
    const handleToggle = () => {
        ...
        setEnabled(newValue);
        try {
            localStorage.setItem('practicepro_content_protection', newValue ? 'true' : 'false');
            // Dispatch a storage event so the useContentProtection hook picks up the change
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    ...
                    <p className="text-xs text-slate-500 dark:text-zinc-400">
                        {enabled
                            ? 'Copy/paste and screenshots are blocked in the app.'
                            : 'You can copy/paste and take screenshots freely.'}
                    </p>
                </div>
            </div>
            <button
                ...
                className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors flex-shrink-0 ${
                    enabled ? 'bg-emerald-600' : 'bg-slate-300 dark:bg-zinc-600'
                }`}
                role="switch"
