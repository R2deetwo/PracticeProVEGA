...
export const TasksView: React.FC = () => {
    ...
    return (
        <div ref={containerRef} className="flex flex-col h-full overflow-hidden bg-surface dark:bg-zinc-900">
            <div className="sticky top-0 pt-safe z-30 glass flex-shrink-0 py-2.5 px-4 sm:px-6 lg:px-8 shadow-sm border-b border-hairline dark:border-zinc-700 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div className="flex items-center gap-4 flex-grow">
                    <h2 className="text-xl sm:text-3xl font-bold text-strong dark:text-white tracking-tight">Tasks</h2>
                    ...
                    <div className="relative flex-grow max-w-sm hidden sm:block">
                        ...
                        <input autoComplete="off" data-lpignore="true" 
                            ...
                            className="w-full pl-9 pr-3 py-1.5 text-sm bg-surface-2/50 dark:bg-zinc-900 border border-hairline dark:border-zinc-700 rounded-lg focus:ring-2 focus:ring-primary-500 transition-shadow"
                        />
                    </div>
                </div>
                ...
                <div className="flex items-center gap-3 w-full sm:w-auto justify-between sm:justify-end">
                    ...
                    <div className="flex items-center gap-2">
                        ...
                        <div className="flex items-center bg-slate-200/50 dark:bg-zinc-800 p-1 rounded-lg border border-hairline dark:border-zinc-700 mr-2">
                            <button
                                onClick={() => setGroupBy('none')}
                                className={`px-2 py-1 text-3xs font-black uppercase tracking-tight rounded-lg transition-all ${groupBy === 'none' ? 'bg-white dark:bg-zinc-700 text-strong dark:text-white shadow-sm' : 'text-subtle hover:text-slate-600'}`}
                            >
                                None
                            </button>
                            <button
                                onClick={() => setGroupBy('status')}
                                className={`px-2 py-1 text-3xs font-black uppercase tracking-tight rounded-lg transition-all ${groupBy === 'status' ? 'bg-white dark:bg-zinc-700 text-strong dark:text-white shadow-sm' : 'text-subtle hover:text-slate-600'}`}
                            >
                                Status
                            </button>
                            <button
                                onClick={() => setGroupBy('priority')}
                                className={`px-2 py-1 text-3xs font-black uppercase tracking-tight rounded-lg transition-all ${groupBy === 'priority' ? 'bg-white dark:bg-zinc-700 text-strong dark:text-white shadow-sm' : 'text-subtle hover:text-slate-600'}`}
                            >
                                Priority
                            </button>
                            <button
                                onClick={() => setGroupBy('matter')}
                                className={`px-2 py-1 text-3xs font-black uppercase tracking-tight rounded-lg transition-all ${groupBy === 'matter' ? 'bg-white dark:bg-zinc-700 text-strong dark:text-white shadow-sm' : 'text-subtle hover:text-slate-600'}`}
                            >
                        ...
                        {hasCompletedTasks && (
                            <button
                                ...
                                className="flex-shrink-0 p-1.5 text-subtle hover:text-red-600 hover:bg-red-50 dark:hover:bg-red-900/20 rounded-lg transition-colors"
