...
export const CHANGELOG: ChangelogEntry[] = [
    {
        ...
        version: '1.18.0',
        date: '2026-07-18',
        title: 'Adobe Acrobat-style PDF Viewer, Full-Screen Preview & Pre-Launch Audit',
        description: 'Documents get a complete preview overhaul — pdf.js-powered viewer with fit-to-page, bottom thumbnails, reading mode, and pinch-to-zoom on mobile. Plus a sweeping pre-launch audit that cleaned up modals, terminology, and product-awareness across every screen.',
        features: [
            { icon: 'document', label: 'Adobe Acrobat-style PDF Viewer', text: 'Replaced the old <object> tag with a pdf.js-powered viewer (same engine as Chrome/Firefox built-in PDF viewers). Canvas-based rendering gives exact fit-to-page math, proper page breaks, and works on iOS Safari, Android Chrome, and desktop.' },
            { icon: 'document', label: 'Full-Screen Preview + Reading Mode', text: 'Click the dark "Full Screen" button on any document to open an immersive reader. Press R for Reading Mode — pure page view with no toolbar, dark background, ESC to exit. Bottom thumbnail strip (Adobe style) for quick page jumps.' },
            { icon: 'pen', label: 'DraftPro: Bidirectional Backlinks', text: 'Use [[Entity Name]] syntax in notes to create backlinks. The Endorsements tab now shows a BacklinksPanel that surfaces every note mentioning the current matter or document — no more lost context.' },
            { icon: 'shield', label: 'Komplete Product-Awareness', text: 'Komplete firms now see BOTH legal and property features everywhere — calendar shows rent due dates AND court deadlines, contacts show Owners/Residents/Legal Clients, Compose defaults to the right audience.' },
            { icon: 'bolt', label: 'Mobile Optimization', text: 'Touch targets bumped to 40px across forms. DraftPro ribbon buttons are now thumb-reachable. Document list kebab menu works on all screen sizes (not just mobile). Form footers wrap properly on narrow screens.' },
            { icon: 'theme', label: 'Pre-Launch UI Audit', text: 'Swept through 25+ files: removed duplicate eyebrows, fixed verbose modal labels (UserForm, ComposeEmailModal, etc.), promoted/demoted modal sizes, made Contacts/Sidebar/BottomNav product-aware. Dead code removed (GoogleDrivePicker, SmartPasteBox).' },
            { icon: 'shield', label: 'Aggressive Cache Busting', text: 'The "Refresh to Update" button now clears Cache Storage, unregisters service workers, and force-reloads with cache-bust query. No more seeing stale UI after a deploy ships.' },
        ],
    },
    {
        id: 'v1.17.0',
