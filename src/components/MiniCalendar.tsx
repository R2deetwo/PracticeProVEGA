...
const MiniCalendar: React.FC<MiniCalendarProps> = ({ events, eventTypes, onEventSelect, onViewFullCalendar }) => {
    ...
    const renderMonthView = () => {
        ...
        return (
            ...
            {/* Headers */}
            <div className="grid grid-cols-7 text-center text-2xs font-bold text-slate-500 dark:text-zinc-400 border-b border-slate-100 dark:border-zinc-700/50 pb-1 mb-1 flex-shrink-0 uppercase tracking-wide">
                            ...
                            <div className="mb-3 bg-slate-50 dark:bg-zinc-800/40 p-2.5 rounded-xl border border-slate-100/50 dark:border-zinc-700/50">
                                ...
                                {tomorrowEvents.slice(0, 2).map(e => (
                                    <button
                                        ...
                                        className="w-full text-left text-2xs font-bold text-slate-700 dark:text-zinc-200 truncate px-2 py-1.5 rounded-lg hover:bg-white dark:hover:bg-zinc-800 transition-all flex items-center gap-2 group/item"
                                    >
                                ...
                                <div className="space-y-1 max-h-[120px] overflow-y-auto pr-1 thin-scrollbar">
                                    {nextWeekEvents.map(e => (
                                        <button
                                            key={e.id}
                                            onClick={() => onEventSelect(e.id, e.date)}
                                            className="w-full text-left text-2xs font-bold text-slate-700 dark:text-zinc-200 truncate px-2 py-1.5 rounded-lg hover:bg-white dark:hover:bg-zinc-800 transition-all border-l-2 border-transparent hover:border-primary-300"
                                        >
                            ...
                            <div className="flex items-start gap-4">
                                ...
                                <div className="flex-grow min-w-0">
                                    ...
                                    <div className="flex items-center gap-2 mt-1.5">
                                        <div className="flex items-center gap-1 px-1.5 py-0.5 rounded bg-slate-100 dark:bg-zinc-700 text-2xs font-bold text-slate-500 dark:text-zinc-400">
                                            <ClockIcon className="w-3 h-3" />
                                            {formatTimeSafe(event.date)}
                                        </div>
                                        {event.matterId && (
                                            <span className="text-2xs text-slate-400 dark:text-zinc-500 truncate font-medium">
                                                Matter Case Ref
                                            </span>
                                        )}
                                    </div>
                    ...
                    {view === 'month' ? (
                        <>
                            ...
                            <button 
                                ...
                                className="px-2 py-0.5 text-2xs font-bold text-primary-600 hover:bg-primary-50 dark:hover:bg-primary-900/30 rounded-md transition-colors"
                            >
                                TODAY
                            </button>
                        ...
                        onViewFullCalendar && activeDate && (
                            <button
                                ...
                                className="px-3 py-1 bg-primary-100 text-primary-700 dark:bg-primary-900/40 dark:text-primary-200 rounded-full font-bold text-2xs hover:bg-primary-200 dark:hover:bg-primary-900/60 transition-colors flex items-center gap-1.5"
                            >
