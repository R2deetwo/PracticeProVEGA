...
const AuthLayout: React.FC<AuthLayoutProps> = ({ title, children }) => {
    ...
    return (
        ...
        >
            ...
            >
                <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-2xl p-8 border border-black/5 dark:border-white/10 relative">
                    <div className="flex justify-center mb-6">
                        <Logo className="h-16 w-auto text-primary-600" />
                    </div>
                    <h2 className="text-2xl font-bold text-center text-slate-900 dark:text-white mb-6">{title}</h2>
                    {children}
                </div>
            </div>
        </div>
