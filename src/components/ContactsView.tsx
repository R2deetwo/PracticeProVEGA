...
const ContactCard: React.FC<{ contact: Contact, onViewDetails: (id: string) => void, isActive?: boolean, isSelected?: boolean, index: number }> = React.memo(({ contact, onViewDetails, isActive, isSelected, index }) => {
    ...
    return (
        <div
            ...
            className={`
            flex items-center justify-between p-3.5 rounded-lg cursor-pointer transition-all duration-200 group border-b border-slate-50 dark:border-zinc-800 last:border-0 min-h-[48px]
...
const ContactListContent: React.FC<{
    ...
    return (
        <div ref={containerRef} className="flex flex-col h-full bg-white dark:bg-zinc-900 border-r border-slate-200 dark:border-zinc-800">
            {/* Header Section */}
            <div className="sticky top-0 pt-safe z-30 glass flex-shrink-0 py-2.5 px-4 sm:px-6 lg:px-8 shadow-sm border-b border-slate-100 dark:border-zinc-800">
