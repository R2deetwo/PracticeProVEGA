...
import { normalizeResearchMessage, UnifiedMessage } from '../../messaging/model';
...
// ─── Bubble slot renderers (ride the shared MessageThread) ────────────────
// Research chat now renders through the SAME thread component as Team and
// Client/Tenant conversations (Path B unification). The pieces that are
// genuinely Research-specific — markdown body, citations, copy affordance,
// thinking dots, branded avatar — are provided as slots on the shared frame.

const ResearchBody: React.FC<{ msg: UnifiedMessage }> = ({ msg }) => {
    const isThinking = (msg.raw as any).isThinking === true;
    if (isThinking) {
        return (
    ...
    const contentHtml = parseAloaMarkdown(msg.content || '');
    return (
        <div
            className="prose prose-sm dark:prose-invert max-w-none break-words selectable overflow-x-auto
                ...
                prose-code:bg-slate-100 dark:bg-zinc-800 dark:prose-code:bg-zinc-700 prose-code:px-1 prose-code:py-0.5 prose-code:rounded prose-code:text-xs prose-code:before:content-none prose-code:after:content-none
...
const ResearchBelow: React.FC<{ msg: UnifiedMessage; sources: ResearchSource[] }> = ({ msg, sources }) => {
    ...
    return (
        <>
            ...
            {!isUser && !isThinking && msg.citations && msg.citations.length > 0 && (
...
export const ResearchChat: React.FC<ResearchChatProps> = ({
    ...
    // Canonical model: researchMessages rows → UnifiedMessage (epoch-ms
    // sentAt regardless of the string timestamps the table stores).
    const unified = React.useMemo(
        () => messages.map(m => normalizeResearchMessage(m)),
        [messages],
    );
    ...
    return (
        <div className="flex flex-col h-full bg-slate-50 dark:bg-zinc-900/50 relative overflow-hidden">
            ...
            {/* Messages — shared MessageThread (variant 'ai'), the same
                thread surface Team and Client/Tenant conversations use. */}
            <MessageThread
                variant="ai"
                threadKey={notebookId}
                messages={unified}
                ...
                renderAvatar={(m) => (
                    <div className={`w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${m.isMe
                            ? 'bg-blue-600 text-white'
                            : 'bg-white dark:bg-zinc-800 border border-slate-200 dark:border-zinc-600 text-blue-600'
                        }`}>
                        {m.isMe ? <UserCircleIcon className="w-4 h-4" /> : <ResearchIcon className="w-4 h-4" />}
                    </div>
                )}
                renderBubbleContent={(m) => <ResearchBody msg={m} />}
                renderBelowBubble={(m) => <ResearchBelow msg={m} sources={sources} />}
            />
