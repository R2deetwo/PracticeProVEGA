...
export const DraftProEditor: React.FC<DraftProEditorProps> = ({
    ...
    // Race-condition guard for handleAiHelpForPlaceholder: each invocation
    // gets a unique ID. After every await, we check whether we're still the
    // "current" request before applying state — older requests are abandoned.
    const aiHelpRequestIdRef = useRef(0);
    // Debounce timer + latest content for the onUpdate → onContentChange
    // pipeline. Each keystroke resets the timer; onContentChange only fires
    // after 500ms of inactivity (or on unmount flush). latestContentRef
    // always holds the most recent HTML so the unmount flush can fire it.
    ...
    const editor = useEditor({
        ...
        onUpdate: ({ editor: e }) => {
            ...
            //
            // DEBOUNCE: We stash the latest HTML in `latestContentRef` and
            // arm a 500ms timer instead of calling onContentChange on every
            // keystroke. Each keystroke clears the prior timer, so onContentChange
            // only fires once typing pauses — this avoids re-rendering the
            // parent (which re-persists the draft to localStorage / Convex)
            // on every character. The unmount cleanup useEffect below
            // flushes any pending content so we never lose the final edit.
            try {
                latestContentRef.current = e.getHTML();
                ...
                contentChangeDebounceRef.current = setTimeout(() => {
                    contentChangeDebounceRef.current = null;
                    try {
                        onContentChange?.(latestContentRef.current);
                    } catch (htmlErr) {
                        console.error('[DraftPro] getHTML failed in debounced onContentChange:', htmlErr);
                    }
                }, 500);
            } catch (htmlErr) {
    ...
    // Unmount flush: if the user navigates away mid-debounce, fire the
    // pending onContentChange immediately so the last edits aren't lost.
    // Without this, typing "Hello" and immediately clicking Back would
    // leave the parent with whatever content was there 500ms ago.
    useEffect(() => {
        return () => {
            if (contentChangeDebounceRef.current) {
                clearTimeout(contentChangeDebounceRef.current);
                ...
                try {
                    ...
                    // Best-effort flush during teardown — don't let a
                    // serialization error here mask the unmount.
                    console.error('[DraftPro] getHTML failed during unmount flush:', e);
                }
            }
        };
    // We intentionally depend only on onContentChange; the refs are stable.
...
--- END CITATION INSTRUCTIONS ---`;
    ...
    const confirmNavWithSave = useCallback(() => {
        ...
        setPendingNav(null);
        cb?.();
    }, [editor, onSave, pendingNav, addToast]);
...
<body>
  ...
  ${html}
    ...
    const handleAiHelpForPlaceholder = (label: string) => {
        ...
        // the meantime, the older request short-circuits instead of stomping
        // state set by the newer one.
        ...
        (async () => {
            try {
                ...
                await streamMessage(
                    ...
                    aiContext,
                    (chunk) => {
                        // Capture chunks even if a newer request is in flight —
                        // we'll discard them via the post-await check below.
                        suggestion += chunk;
                    },
                    'flash'
                );
                ...
                // otherwise a newer placeholder request has its own loading flag
                // set, and clearing it here would prematurely hide its spinner.
