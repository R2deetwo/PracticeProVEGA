...
const NotebookForm: React.FC<NotebookFormProps> = ({ onAdd, onUpdate, onClose, notebookToEdit, currentUser, appMode }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <label htmlFor="notebookName" className="block text-sm font-medium text-slate-700 dark:text-dim-300 mb-1">Notebook Name</label>
      ...
      {appMode === 'multi' && (
        <div>
          <label className="block text-sm font-medium text-slate-700 dark:text-dim-300 mb-1">Visibility</label>
          <div className="flex space-x-4 rounded-lg bg-slate-100 dark:bg-dim-700 p-1">
            <button type="button" onClick={() => setScope(NoteScope.Firm)} className={`w-full text-center px-4 py-2 rounded-md text-sm font-semibold transition-colors ${scope === NoteScope.Firm ? 'bg-white dark:bg-dim-800 shadow' : 'text-dim-600 dark:text-dim-300'}`}>
              Firm
            </button>
            <button type="button" onClick={() => setScope(NoteScope.Private)} className={`w-full text-center px-4 py-2 rounded-md text-sm font-semibold transition-colors ${scope === NoteScope.Private ? 'bg-white dark:bg-dim-800 shadow' : 'text-dim-600 dark:text-dim-300'}`}>
              Private
            </button>
          </div>
          <p className="text-xs text-slate-500 dark:text-dim-400 mt-1">
