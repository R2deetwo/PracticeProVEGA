  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <form onSubmit={handleSubmit} className="space-y-3 md:col-span-2">
            <div>
                <label htmlFor="templateName" className="block text-sm font-medium text-slate-700 dark:text-dim-300 mb-1">Template Name</label>
             ...
             <div>
                <label htmlFor="templateCategory" className="block text-sm font-medium text-slate-700 dark:text-dim-300 mb-1">Category</label>
            ...
            <div>
                <label htmlFor="templateContent" className="block text-sm font-medium text-slate-700 dark:text-dim-300 mb-1">Template Content</label>
                <textarea
                id="templateContent"
                value={content}
        ...
        <div className="md:col-span-1 p-4 bg-slate-50 dark:bg-dim-700/50 rounded-lg">
            <h4 className="font-semibold text-slate-800 dark:text-dim-200 mb-2">Placeholders</h4>
            <p className="text-xs text-slate-500 dark:text-dim-400 mb-3">Click a placeholder to copy it.</p>
                ...
                {TEMPLATE_PLACEHOLDERS.map(p => (
                    <Tooltip key={p.placeholder} text={p.description}>
                        <button 
                            ...
                            className="w-full text-left p-2 bg-white dark:bg-dim-700 rounded-md font-mono text-xs text-primary-700 dark:text-primary-300 hover:bg-primary-50 dark:hover:bg-primary-900/30 dark:hover:bg-primary-900/40 transition-colors"
                        >
                            {p.placeholder}
                        </button>
                    </Tooltip>
