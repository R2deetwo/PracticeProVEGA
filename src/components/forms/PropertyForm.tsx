...
const AccordionSectionInner: React.FC<AccordionSectionProps> = ({ id, title, subtitle, icon, iconBg, children, isOpen, onToggle, onFocusSection }) => {
    ...
    return (
        <div
            ...
            // — CSS containment was unnecessary here and can interfere with pointer
            // events in some browsers. The accordion works fine without it.
        ...
        >
            <button
                ...
                className="w-full flex items-center gap-3 p-2.5 sm:p-3 hover:bg-slate-100/50 dark:hover:bg-zinc-700/30 transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500/30"
            >
...
const PropertyForm: React.FC<PropertyFormProps> = ({ contact, propertyToEdit, activeUnitId, autoExpandRental, autoAddUnit, onSave, onClose }) => {
        ...
        if (propertyToEdit) {
            const allUnits = (coreState.properties || [])
                ...
                .map(p => {
                    ...
                    let lf = Number(rd.legalFee) || 0;
                    let af = Number(rd.agencyFee) || 0;
                    ...
                    // Amount-first healing: legacy rows could be saved with a
                    // percentage but a zero amount (the old % input only
                    // recomputed the amount on rent/pct edits). Reconstruct the
    ...
    // (Round-4: the % string buffers were removed with the % inputs — the
    //  amount inputs format directly through formatNumberWithCommas.)
    ...
    // (Round-4: disputeStatus state removed — it had no input UI and no
    //  readers anywhere; the linked matter owns the dispute's real status.)
    ...
    const updateUnit = (index: number, field: keyof UnitRentalInput, value: any) => {
        setUnitsData(prev => {
            ...
            // AMOUNT-FIRST fee model (round-4): the naira amounts are the source
            // of truth; the legacy percentage fields are derived so older
            // readers (letters, placeholder registry) keep working.
            const derivePct = (amount: number, rent: number) =>
                rent > 0 ? Math.round((amount / rent) * 100) : 0;

            // Rent changed → re-derive percentages from the stored amounts.
            if (field === 'rentAmount') {
                const rent = Number(value) || 0;
                newUnits[index].legalFeePercentage = newUnits[index].isLegalNA ? 0 : derivePct(newUnits[index].legalFee || 0, rent);
                newUnits[index].agencyFeePercentage = newUnits[index].isAgencyNA ? 0 : derivePct(newUnits[index].agencyFee || 0, rent);
            }
            // Amount typed → re-derive its percentage.
            if (field === 'legalFee') {
                newUnits[index].legalFeePercentage = newUnits[index].isLegalNA ? 0 : derivePct(Number(value) || 0, newUnits[index].rentAmount || 0);
            }
            if (field === 'agencyFee') {
                newUnits[index].agencyFeePercentage = newUnits[index].isAgencyNA ? 0 : derivePct(Number(value) || 0, newUnits[index].rentAmount || 0);
            }
            // N/A toggled → zero both the amount and its percentage.
            if (field === 'isLegalNA') {
                if (value) { newUnits[index].legalFee = 0; newUnits[index].legalFeePercentage = 0; }
            }
            if (field === 'isAgencyNA') {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const propertyData: Partial<Property> = {
            ...
            // ALL services are configurable, service charge included — the
            // manager decides which services exist for this property. Turning
            // a service off grays it out in the Resident Portal.
            coreServices,
            value,
        ...
        if (category === 'Disputed Property') {
            propertyData.disputeDetails = {
                ...
                court,
                opposingParty,
                status: propertyToEdit?.disputeDetails?.status
            };
        }
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                            ...
                            <div className="space-y-2 group md:col-span-2">
                                ...
                                <p className="text-3xs text-slate-400 dark:text-zinc-500 mb-2">
                                    Toggle which services are active for this property — every service can be turned off. Inactive services appear grayed out in the Resident Portal.
                                </p>
                                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                                    {([
                                        ...
                                        <button
                                            ...
                                            className={`flex items-center justify-between px-3 py-2 rounded-lg border text-xs font-bold transition-all cursor-pointer ${
                                            ...
                                            title={coreServices[service.key] ? `Deactivate ${service.label} for this property` : `Activate ${service.label} for this property`}
                                        >
                        ...
                        <div className="space-y-3 sm:space-y-4 pt-1 animate-fade-in" key={unitsData[activeUnitIndex]?.id || activeUnitIndex}>
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-3 sm:gap-4">
                                ...
                                {rentCollectionMode === 'Management Only (No Rent)' ? (
                                    <div className="md:col-span-2 p-4 rounded-lg bg-slate-50 dark:bg-zinc-800/50 border border-slate-200 dark:border-zinc-700">
