...
const DocumentCategoryForm: React.FC<DocumentCategoryFormProps> = ({ onAddCategory, onUpdateCategory, onDelete, onClose, categoryToEdit, allCategories, context }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        <label htmlFor="docCatName" className="block text-sm font-medium text-slate-700 dark:text-dim-300 mb-1">Category Name</label>
      ...
      <div>
        <label htmlFor="docCatParent" className="block text-sm font-medium text-slate-700 dark:text-dim-300 mb-1">Parent Category (optional)</label>
        <select 
            id="docCatParent"
            value={parentId || ''}
