...
interface ContactCategoryFormProps {
  onAddCategory?: (newCategory: Omit<ContactCategory, 'id'>) => void;
  onUpdateCategory?: (updatedCategory: ContactCategory) => void;
  onDelete?: () => void;
  onClose: () => void;
  categoryToEdit?: ContactCategory;
}
...
const ContactCategoryForm: React.FC<ContactCategoryFormProps> = ({ onAddCategory, onUpdateCategory, onDelete, onClose, categoryToEdit }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (isEditing && onUpdateCategory && categoryToEdit) {
        /* Added firmId to satisfy ContactCategory interface */
        const categoryData: ContactCategory = {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-3">
      <div>
        ...
        <input autoComplete="off" data-lpignore="true"  type="text" id="categoryName" value={name} onChange={e => setName(e.target.value)} placeholder="e.g., Expert Witness" className={commonInputClass} required />
      </div>
