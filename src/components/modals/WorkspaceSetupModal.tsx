    ...
    <div className="space-y-6">
            ...
            <form onSubmit={handleSubmit} className="space-y-5 animate-fade-in">
                <div>
                    ...
                    <div className="relative">
                        ...
                        <input autoComplete="off" data-lpignore="true"
                            ...
                            className="w-full pl-10 pr-4 py-3 border border-slate-200 dark:border-zinc-700 rounded-xl bg-white dark:bg-zinc-800 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 shadow-sm text-slate-900 dark:text-white"
                ...
                <div>
                    ...
                    <div className="grid grid-cols-3 gap-2">
                        <button type="button" onClick={() => setProduct('legal')} className={`p-3 text-center border rounded-xl transition-all ${product === 'legal' ? 'border-primary-500 bg-primary-50 dark:bg-primary-900/20 ring-1 ring-primary-500' : 'border-slate-200 dark:border-zinc-700 hover:border-slate-300'}`}>
                            <div className="font-bold text-xs text-slate-900 dark:text-white">Vega (Legal)</div>
                        </button>
                        <button type="button" onClick={() => setProduct('property')} className={`p-3 text-center border rounded-xl transition-all ${product === 'property' ? 'border-emerald-500 bg-emerald-50 dark:bg-emerald-900/20 ring-1 ring-emerald-500' : 'border-slate-200 dark:border-zinc-700 hover:border-slate-300'}`}>
                            <div className="font-bold text-xs text-slate-900 dark:text-white">Atrium (Property)</div>
                        </button>
                        <button type="button" onClick={() => setProduct('unified')} className={`p-3 text-center border rounded-xl transition-all ${product === 'unified' ? 'border-indigo-500 bg-indigo-50 dark:bg-indigo-900/20 ring-1 ring-indigo-500' : 'border-slate-200 dark:border-zinc-700 hover:border-slate-300'}`}>
                ...
                <div className="flex justify-end gap-2 pt-1">
                    <button 
                        type="button" 
                        onClick={onClose} 
                        className="px-4 py-2.5 text-slate-600 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-800 rounded-xl font-bold text-sm transition-colors"
                    >
                        Cancel
                    </button>
                    <button 
        ...
        {status === 'completed' && (
            <div className="flex flex-col items-center justify-center py-6 animate-fade-in">
                ...
                <p className="text-lg font-black text-slate-800 dark:text-white">You're all set!</p>
                <p className="text-sm text-slate-500 dark:text-zinc-400">Saving your {pendingAction?.toLowerCase() || 'item'} now...</p>
            </div>
        )}
    </div>
  );
