 ...
 return (
  <div className="flex flex-col h-full animate-fade-in" onKeyDown={handleKeyDown}>
   ...
   <div className="p-5 sm:p-6 flex items-start gap-4">
    ...
    <button
     onClick={onCancel}
     className="flex-shrink-0 p-1 rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition-colors"
     aria-label="Close"
    >
     <X className="w-4 h-4" />
    </button>
     ...
     {onExport && (
      <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg flex items-center justify-between">
       ...
       <button
        type="button"
        onClick={onExport}
        className="flex items-center gap-2 px-3 py-1.5 bg-white border border-blue-300 text-blue-700 text-xs font-bold rounded shadow-sm hover:bg-blue-50"
       >
     ...
     {verificationText && !requiresPassword && (
      <div className="p-4 bg-rose-50 rounded-lg border border-rose-100">
       <label className="block text-sm font-semibold text-rose-800 mb-2 select-none">
        To confirm, type <span className="font-mono bg-white px-1 py-0.5 rounded border border-rose-200 select-all">{verificationText}</span> below:
       </label>
       <input
        ref={inputRef}
        type="text"
        ...
        className="w-full p-2.5 text-sm border border-rose-300 rounded-lg focus:ring-2 focus:ring-rose-500 focus:border-rose-500 bg-white text-slate-900"
     ...
     {requiresPassword && (
      <div>
       ...
       <input
        ...
        value={inputValue}
        onChange={(e) => setInputValue(e.target.value)}
        className="w-full px-4 py-2.5 text-sm border border-slate-300 rounded-xl bg-white text-slate-900 focus:ring-2 focus:ring-rose-500 focus:border-transparent"
        autoComplete="off"
       />
      </div>
     )}
   ...
   <div className="px-5 sm:px-6 py-4 bg-slate-50 border-t border-slate-200 flex flex-col-reverse sm:flex-row sm:justify-end gap-2 mt-auto">
    <button
     type="button"
     onClick={onCancel}
     className="px-4 py-2.5 rounded-xl text-sm font-semibold text-slate-700 bg-white border border-slate-200 hover:bg-slate-50 transition-colors active:scale-[0.98]"
    >
     Cancel
    </button>
