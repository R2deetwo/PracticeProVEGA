...
const FolderPermissionsModal: React.FC<FolderPermissionsModalProps> = ({ folder, allRoles, currentPermissions, onUpdatePermissions, onClose }) => {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
        <div>
            <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Grant Access to:</label>
            <div className="space-y-2 p-3 border border-gray-200 dark:border-gray-600 rounded-md">
            {allRoles.map(role => (
                <label key={role} className="flex items-center space-x-3 cursor-pointer">
                    ...
                    />
                    <span className="text-sm text-gray-900 dark:text-gray-200">{role}</span>
                </label>
            ))}
            </div>
        </div>
