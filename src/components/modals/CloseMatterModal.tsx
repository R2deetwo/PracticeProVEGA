...
const CloseMatterModal: React.FC<CloseMatterModalProps> = ({ matter, unbilledTime, unbilledExpenses, onConfirm, onClose }) => {
 ...
 return (
  <form onSubmit={handleSubmit} className="space-y-4">
    ...
    <div>
      <label htmlFor="closingNote" className="block text-xs font-bold text-slate-500 uppercase tracking-wider mb-1.5">Closing Summary (Optional)</label>
      <textarea
        id="closingNote"
        rows={4}
        value={closingNote}
        ...
        className="text-slate-900 w-full bg-white border border-slate-200 rounded-xl shadow-sm p-3 focus:ring-2 focus:ring-primary-500 focus:border-primary-500 text-sm resize-none outline-none"
      />
    </div>
    <div className="pt-4 flex justify-end space-x-2">
      <button type="button" onClick={onClose} className="px-4 py-2 bg-slate-100 text-slate-700 rounded-xl font-bold text-sm hover:bg-slate-200 transition-colors">Cancel</button>
