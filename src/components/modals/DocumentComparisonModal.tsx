...
export const DocumentComparisonModal: React.FC<DocumentComparisonModalProps> = ({ currentDocument, versionToCompare, onClose }) => {
  return (
    <div className="flex flex-col items-center justify-center py-12 text-center">
      ...
      <h4 className="text-lg font-bold text-slate-800 mb-2">Version Comparison</h4>
      <p className="text-sm text-slate-500 max-w-md leading-relaxed mb-3">
        Comparing <strong className="text-slate-700">{currentDocument.title}</strong> — current version vs. version from <strong className="text-slate-700">{new Date(versionToCompare.uploadedAt).toLocaleDateString('en-GB')}</strong>.
      </p>
