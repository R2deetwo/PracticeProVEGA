    ...
    <div className="space-y-6 pb-2">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50 dark:bg-zinc-900/50 p-4 rounded-2xl border border-slate-100 dark:border-zinc-800">
         <div className="flex items-start gap-3">
          <div className="p-2 bg-white dark:bg-zinc-900 rounded-xl shadow-sm text-primary-600">
      ...
      <div className="flex flex-wrap gap-2 pt-2">
        {showViewInTasksButton && (
           <button type="button" onClick={() => onViewInTasks(task.id, 'blue')} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 rounded-xl text-3xs font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
            <TasksIcon className="w-3.5 h-3.5" /> LOCATE
          </button>
        )}
        {task.dueDate && openedFrom !== 'calendar' && onNavigateToCalendar && (
          <button type="button" onClick={() => onNavigateToCalendar(new Date(task.dueDate!).toISOString().split('T')[0], task.id)} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 rounded-xl text-3xs font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
