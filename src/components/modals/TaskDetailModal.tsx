...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = (props) => {
  ...
  return (
    <div className="space-y-6 pb-2">
      {isOverdue && (
        <div className="p-4 bg-red-50 border-l-4 border-red-500 text-red-800 rounded-r-xl animate-fade-in shadow-sm">
      ...
      <div className="flex flex-col gap-1 border-b border-slate-100 pb-4">
         <div className="flex items-center gap-2 mb-1">
          <span className={`px-2 py-0.5 text-2xs font-black uppercase tracking-widest rounded-md ${task.status === 'done' ? 'bg-green-100 text-green-700' : 'bg-amber-100 text-amber-700'}`}>
            {task.status.replace('_', ' ')}
          </span>
          <span className={`px-2 py-0.5 bg-slate-100 text-slate-600 text-2xs font-black uppercase tracking-widest rounded-md`}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50/50 p-4 rounded-2xl border border-slate-100">
         ...
         <div className="flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm font-bold text-slate-700">Legal Task</p>
          </div>
         </div>
      </div>
      ...
      {task.description && (
        <div className="space-y-2">
          ...
          <div className="p-4 bg-white border border-slate-100 rounded-2xl text-sm text-slate-700 shadow-sm leading-relaxed italic">
            {task.description}
          </div>
        </div>
      )}
      ...
      {matter && (
        <div className="p-4 glass-premium rounded-2xl flex items-center justify-between border-l-4 border-l-primary-500">
          <div className="min-w-0">
            ...
            <p className="text-sm font-bold text-slate-800 truncate pr-4">{matter.title}</p>
          </div>
          {openedFrom !== 'matterDetail' && onNavigateToMatter && (
             <button type="button" onClick={() => onNavigateToMatter(matter.id, task.id)} className="px-3 py-1 bg-primary-100 text-primary-700 rounded-lg text-3xs font-black uppercase tracking-widest hover:bg-primary-600 hover:text-white transition-all">
              OPEN
            </button>
          )}
        </div>
      ...
      <div className="flex flex-wrap gap-2 pt-2">
        {showViewInTasksButton && (
           <button type="button" onClick={() => onViewInTasks(task.id, 'blue')} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-600 rounded-xl text-3xs font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
            <TasksIcon className="w-3.5 h-3.5" /> LOCATE
          </button>
        )}
        {task.dueDate && openedFrom !== 'calendar' && onNavigateToCalendar && (
          <button type="button" onClick={() => onNavigateToCalendar(new Date(task.dueDate!).toISOString().split('T')[0], task.id)} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 text-slate-600 rounded-xl text-3xs font-black uppercase tracking-widest hover:bg-slate-200 transition-all">
      ...
      <div className="pt-4 border-t border-slate-100 flex justify-between items-center gap-3">
        {canManage && (
          <>
            <button type="button" onClick={onDelete} className="px-6 py-2.5 bg-red-50 text-red-600 rounded-xl font-black text-2xs uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all shadow-sm flex items-center gap-2">
