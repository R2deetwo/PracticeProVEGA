...
export const TaskDetailModal: React.FC<TaskDetailModalProps> = (props) => {
    ...
    return (
        <div className="space-y-6 pb-2">
            {isOverdue && (
                <div className="p-4 bg-red-50 dark:bg-red-900/20 border-l-4 border-red-500 text-red-800 dark:text-red-200 rounded-r-xl animate-fade-in shadow-sm">
            ...
            <div className="flex flex-col gap-1 border-b border-slate-100 dark:border-zinc-700/50 pb-4">
                 <div className="flex items-center gap-2 mb-1">
                    <span className={`px-2 py-0.5 text-[10px] font-black uppercase tracking-widest rounded-md ${task.status === 'done' ? 'bg-green-100 text-green-700 dark:bg-green-900/40 dark:text-green-400' : 'bg-amber-100 text-amber-700 dark:bg-amber-900/40 dark:text-amber-400'}`}>
                        {task.status.replace('_', ' ')}
                    </span>
                    <span className={`px-2 py-0.5 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 text-[10px] font-black uppercase tracking-widest rounded-md`}>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 bg-slate-50/50 dark:bg-zinc-800/20 p-4 rounded-2xl border border-slate-100 dark:border-zinc-700/30">
                 <div className="flex items-start gap-3">
                    ...
                    <div>
                       <p className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.2em] mb-0.5">DUE DATE</p>
                 ...
                 <div className="flex items-start gap-3">
                    ...
                    <div>
                       <p className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.2em] mb-0.5">TASK CATEGORY</p>
                        <p className="text-sm font-bold text-slate-700 dark:text-slate-200">Legal Task</p>
                    </div>
                 </div>
            </div>
            ...
            {task.description && (
                <div className="space-y-2">
                    <p className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.2em] px-1">SPECIFICATIONS</p>
            ...
            {matter && (
                <div className="p-4 glass-premium rounded-2xl flex items-center justify-between border-l-4 border-l-primary-500">
                    <div className="min-w-0">
                        ...
                        <p className="text-sm font-bold text-slate-800 dark:text-white truncate pr-4">{matter.title}</p>
                    </div>
                    {openedFrom !== 'matterDetail' && onNavigateToMatter && (
                         <button type="button" onClick={() => onNavigateToMatter(matter.id, task.id)} className="px-3 py-1 bg-primary-100 dark:bg-primary-900/40 text-primary-700 dark:text-primary-300 rounded-lg text-[9px] font-black uppercase tracking-widest hover:bg-primary-600 hover:text-white transition-all">
                            OPEN
                        </button>
                    )}
                </div>
            ...
            <div className="space-y-3">
                <p className="text-[10px] font-black text-slate-400 dark:text-zinc-500 uppercase tracking-[0.2em] px-1">ASSIGNED TEAM</p>
                        ...
                        <div className="flex items-center gap-3">
                            <div className="flex items-center -space-x-2">
                                {assignedUsers.slice(0, 5).map(user => (
                                    <div key={user.id} title={user.name} className={`h-8 w-8 rounded-full ring-4 ring-white dark:ring-zinc-900 flex items-center justify-center text-white font-bold text-xs shadow-sm ${getUserColor(user.id)}`}>
            ...
            <div className="flex flex-wrap gap-2 pt-2">
                {showViewInTasksButton && (
                     <button type="button" onClick={() => onViewInTasks(task.id, 'blue')} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-zinc-700 transition-all">
                        <TasksIcon className="w-3.5 h-3.5" /> LOCATE
                    </button>
                )}
                {task.dueDate && openedFrom !== 'calendar' && onNavigateToCalendar && (
                    <button type="button" onClick={() => onNavigateToCalendar(new Date(task.dueDate!).toISOString().split('T')[0], task.id)} className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-100 dark:bg-zinc-800 text-slate-600 dark:text-zinc-400 rounded-xl text-[9px] font-black uppercase tracking-widest hover:bg-slate-200 dark:hover:bg-zinc-700 transition-all">
            ...
            <div className="pt-4 border-t border-slate-100 dark:border-zinc-800 flex justify-between items-center gap-3">
                {canManage && (
                    <>
                        <button type="button" onClick={onDelete} className="px-6 py-2.5 bg-red-50 text-red-600 dark:bg-red-900/20 dark:text-red-400 rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-red-600 hover:text-white transition-all shadow-sm flex items-center gap-2">
                            <TrashIcon className="w-3.5 h-3.5" /> Delete
                        </button>
                        <button type="button" onClick={onEdit} className="flex-grow py-2.5 bg-primary-600 text-white rounded-xl font-black text-[10px] uppercase tracking-widest hover:bg-primary-700 transition-all shadow-lg hover:shadow-primary-500/20 active:scale-[0.98] flex items-center justify-center gap-2">
