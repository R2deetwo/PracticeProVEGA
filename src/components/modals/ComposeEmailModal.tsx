...
const ComposeEmailModal: React.FC<ComposeEmailModalProps> = ({ onClose, initialContext }) => {
  ...
  return (
    <div className="flex flex-col -m-1 sm:-m-2 h-[75vh] sm:h-auto sm:max-h-[80vh]">
      ...
      {confirmNoSubject && (
        <div className="mx-1 mt-1 p-3 bg-amber-50 border border-amber-200 rounded-xl flex items-center justify-between gap-3 animate-fade-in">
          ...
          <div className="flex gap-2 flex-shrink-0">
            <button
              ...
              className="px-3 py-1 text-xs font-bold text-amber-700 bg-amber-100 rounded-lg hover:bg-amber-200 transition-colors"
            >
              Cancel
            </button>
            <button
      ...
      <div className="flex-shrink-0 z-10">
        <div className="flex items-center border-b border-slate-100 py-2">
          <span className="text-slate-400 text-xs font-bold w-14">To</span>
          <input autoComplete="off" data-lpignore="true"
            type="text"
            value={to}
            onChange={e => setTo(e.target.value)}
            className="flex-grow bg-transparent border-none focus:ring-0 text-sm py-1 text-slate-900 placeholder-slate-400 outline-none"
            placeholder="Recipients"
            autoFocus
          />
          <div className="flex gap-3 text-xs font-bold text-slate-400">
            <button type="button" onClick={() => setShowCc(!showCc)} className="hover:text-primary-600 transition-colors">Cc</button>
            <button type="button" onClick={() => setShowBcc(!showBcc)} className="hover:text-primary-600 transition-colors">Bcc</button>
          </div>
        </div>

        {showCc && (
          <div className="flex items-center border-b border-slate-100 py-2 animate-fade-in">
            <span className="text-slate-400 text-xs font-bold w-14">Cc</span>
            <input autoComplete="off" data-lpignore="true"
              type="text"
              value={cc}
              onChange={e => setCc(e.target.value)}
              className="flex-grow bg-transparent border-none focus:ring-0 text-sm py-1 text-slate-900 outline-none"
            />
          </div>
        )}
        ...
        {showBcc && (
          <div className="flex items-center border-b border-slate-100 py-2 animate-fade-in">
            <span className="text-slate-400 text-xs font-bold w-14">Bcc</span>
            <input autoComplete="off" data-lpignore="true"
              type="text"
              value={bcc}
        ...
        <div className="flex items-center border-b border-slate-100 py-2">
          <span className="text-slate-400 text-xs font-bold w-14">Subject</span>
          <input autoComplete="off" data-lpignore="true"
            type="text"
            value={subject}
            onChange={e => { setSubject(e.target.value); setConfirmNoSubject(false); }}
            className="flex-grow bg-transparent border-none focus:ring-0 text-sm py-1 font-bold text-slate-900 outline-none"
            placeholder="Subject"
          />
        </div>
      </div>
      ...
      <div className="flex-grow overflow-y-auto custom-scrollbar relative">
        <textarea
          ...
          className="w-full h-full resize-none border-none focus:ring-0 bg-transparent text-sm leading-relaxed text-slate-800 placeholder-slate-400 outline-none p-1"
      ...
      {attachments.length > 0 && (
        <div className="py-2 flex flex-wrap gap-2 flex-shrink-0 border-t border-slate-100">
          {attachments.map(doc => (
            <div key={doc.id} className="flex items-center gap-1.5 bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs group">
              <DocumentIcon className="w-3.5 h-3.5 text-red-500" />
              <span className="truncate max-w-[160px] font-medium text-slate-700">{doc.title}</span>
      ...
      <div className="pt-3 border-t border-slate-200 flex justify-between items-center flex-shrink-0">
        <div className="relative">
          <button
            ...
            className={`p-2 rounded-xl transition-colors ${isAttaching ? 'bg-primary-100 text-primary-600' : 'text-slate-400 hover:bg-slate-100 hover:text-slate-600'}`}
          ...
          {isAttaching && (
            <div className="absolute bottom-full left-0 mb-2 w-72 bg-white rounded-xl shadow-2xl border border-slate-200 overflow-hidden z-50 animate-fade-in">
              <div className="px-4 py-2.5 text-2xs font-black text-slate-400 uppercase tracking-widest bg-slate-50 border-b border-slate-100">
                Attach from Matter
              </div>
                ...
                {matterDocs.length > 0 ? matterDocs.map(doc => {
                  ...
                  return (
                    <button
                      ...
                      className={`w-full text-left px-4 py-2.5 text-sm truncate flex items-center gap-3 transition-colors ${isAttached ? 'bg-primary-50 text-primary-700' : 'hover:bg-slate-50 text-slate-700'}`}
                    >
                      <div className={`w-4 h-4 rounded border flex items-center justify-center flex-shrink-0 ${isAttached ? 'bg-primary-600 border-primary-600' : 'border-slate-300'}`}>
        ...
        <div className="flex gap-2">
          <button type="button" onClick={onClose} className="px-4 py-2 text-slate-500 text-xs font-bold hover:bg-slate-100 rounded-xl transition-colors">
            Cancel
          </button>
          <button
            type="button"
