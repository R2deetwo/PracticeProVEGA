...
const TabButton: React.FC<{ label: string; icon: React.ReactNode; isActive: boolean; onClick: () => void }> = ({ label, icon, isActive, onClick }) => (
    <button
        ...
        className={`flex-1 flex items-center justify-center gap-2 py-2.5 text-sm font-medium rounded-lg transition-all ${isActive
                ? 'bg-white dark:bg-zinc-700 text-primary-600 dark:text-primary-400 shadow-sm ring-1 ring-slate-200 dark:ring-zinc-600'
                : 'text-slate-500 hover:text-slate-700 dark:text-zinc-400 hover:bg-slate-100 dark:hover:bg-zinc-700/50'
            }`}
    >
        {icon}
        {label}
...
const AddResearchSourceModal: React.FC<AddResearchSourceModalProps> = ({ notebookId, onAdd, onClose }) => {
    ...
    return (
        <form onSubmit={handleSubmit} className="space-y-6">
            <div className="flex p-1 bg-slate-100 dark:bg-zinc-800/50 rounded-xl border border-slate-200 dark:border-zinc-700">
            ...
            {activeTab === 'website' && (
                <div className="space-y-4 animate-fade-in">
                    ...
                    <div>
                        <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Source Name (Optional)</label>
            ...
            {activeTab === 'upload' && (
                <div className="space-y-4 animate-fade-in">
                    ...
                    <div
                        className="border-2 border-dashed border-gray-300 dark:border-zinc-600 rounded-xl p-8 text-center hover:bg-slate-50 dark:hover:bg-zinc-800/50 transition-colors cursor-pointer group"
                    ...
                    >
                        ...
                        <div className="flex flex-col items-center">
                            <div className="p-3 bg-slate-100 dark:bg-zinc-700 rounded-full mb-3 group-hover:scale-110 transition-transform">
                    ...
                    {files.length > 0 && (
                        <div className="space-y-2 max-h-40 overflow-y-auto custom-scrollbar">
                            {files.map((file, idx) => (
                                <div key={idx} className="flex justify-between items-center p-2 bg-slate-50 dark:bg-zinc-800 rounded border border-slate-200 dark:border-zinc-700">
            ...
            <div className="pt-4 flex justify-end gap-3 border-t border-slate-100 dark:border-zinc-700">
                <button type="button" onClick={onClose} className="px-5 py-2 bg-white dark:bg-zinc-700 border border-slate-300 dark:border-zinc-600 text-slate-700 dark:text-zinc-200 rounded-lg font-semibold hover:bg-slate-50 dark:hover:bg-zinc-600 transition-colors">Cancel</button>
                <button
                    type="submit"
                    disabled={processing}
