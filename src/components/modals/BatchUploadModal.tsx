...
const BatchUploadModal: React.FC<BatchUploadModalProps> = ({ files: initialFiles, context, onClose }) => {
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="max-h-80 overflow-y-auto space-y-2 border border-slate-200 dark:border-zinc-700/50 rounded-xl p-3 bg-slate-50/50 dark:bg-zinc-900/50 backdrop-blur-sm custom-scrollbar">
                {uploads.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-3 bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-slate-100 dark:border-zinc-700 transition-all hover:border-primary-500/50 dark:hover:border-primary-500/50">
                        <div className="w-10 h-10 bg-slate-100 dark:bg-zinc-700 rounded-lg flex items-center justify-center flex-shrink-0">
                            <DocumentIcon className="w-5 h-5 text-slate-500 dark:text-slate-300" />
                        </div>
                        <div className="flex-1 min-w-0">
                            ...
                            <div className="w-full bg-slate-200 dark:bg-zinc-700 rounded-full h-1 overflow-hidden">
                                <div
                                    className={`h-full transition-all duration-700 ease-out ${item.status === 'failed' ? 'bg-red-500' : 'bg-primary-600 dark:bg-primary-500'
                                        }`}
                        ...
                        <div className="flex items-center gap-1">
                            {item.status === 'failed' && (
                                <button onClick={() => handleRetry(idx)} className="p-2 text-slate-500 hover:text-primary-600 rounded-lg hover:bg-primary-50 dark:hover:bg-primary-900/20" title="Retry">
                                    <RevertIcon className="w-4 h-4" />
                                </button>
                            )}

                            {(item.status === 'pending' || item.status === 'failed') && (
                                <button onClick={() => handleRemove(idx)} className="p-2 text-slate-400 hover:text-red-500 rounded-lg hover:bg-red-50 dark:hover:bg-red-900/20">
            ...
            <div className="p-4 bg-slate-50 dark:bg-zinc-800/50 rounded-xl border border-slate-200 dark:border-zinc-700">
                <div className="flex items-center justify-between">
                    <div>
                        <label className="block text-sm font-bold text-slate-800 dark:text-white">Mark all as Court Processes</label>
                    ...
                    <button
                        ...
                        className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors focus:outline-none ring-2 ring-primary-500/10 ${isCourtProcess ? 'bg-primary-600' : 'bg-slate-300 dark:bg-zinc-600'} ${isProcessing ? 'opacity-50' : ''}`}
                    >
