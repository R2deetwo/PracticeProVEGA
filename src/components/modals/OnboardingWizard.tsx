    ...
    >
      ...
      <div className="p-5 flex flex-col h-full">
        ...
        {tier.requiresSetupFee && (
          <p className="text-3xs text-amber-600 font-black uppercase tracking-widest mt-3 border-t border-slate-100 dark:border-zinc-800 pt-2">+ ₦150k One-Time Setup Fee</p>
        )}
      </div>
    </div>
  );
          ...
          <div className="space-y-8">
            ...
            <div className="max-w-md mx-auto space-y-6">
              <div className="flex p-1.5 bg-slate-50 dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800">
                <button onClick={() => { setMode('create'); setError(null); }} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${mode === 'create' ? 'bg-white dark:bg-zinc-900 shadow-lg shadow-slate-200 text-primary-600' : 'text-slate-400'}`}>Create New</button>
                <button onClick={() => { setMode('join'); setError(null); }} className={`flex-1 py-3 text-xs font-black uppercase tracking-widest rounded-xl transition-all ${mode === 'join' ? 'bg-white dark:bg-zinc-900 shadow-lg shadow-slate-200 text-primary-600' : 'text-slate-400'}`}>Join Existing</button>
              </div>
        ...
        {step === 2 && (
          <div className="space-y-6">
            ...
            {!userProduct && (
              <div className="flex justify-center gap-3">
                {(['legal', 'property', 'unified'] as ProductMode[]).map(p => {
                  ...
                  return (
                    <button key={p} onClick={() => setProduct(p)} className={`px-5 py-2 text-2xs font-black uppercase tracking-widest rounded-full border-2 transition-all ${active ? 'border-primary-500 bg-primary-50 text-primary-700' : 'border-slate-200 dark:border-zinc-700 text-slate-400 hover:border-slate-300'}`}>
                      {name}
                    </button>
                  );
                })}
            ...
            <div className="max-w-md mx-auto pt-2 flex items-start gap-3">
              <input autoComplete="off" data-lpignore="true" type="checkbox" id="agree-dpa" checked={hasAgreed} onChange={e => setHasAgreed(e.target.checked)} className="mt-0.5 w-4 h-4 text-primary-600 border-slate-200 dark:border-zinc-700 rounded focus:ring-primary-500 cursor-pointer transition-all" />
            ...
            <div className="flex flex-col sm:flex-row gap-4 max-w-md mx-auto pt-2">
              <button onClick={() => setStep(1)} className="flex-1 py-4 bg-slate-50 dark:bg-zinc-900 text-slate-400 font-black text-xs uppercase tracking-widest rounded-2xl hover:bg-slate-100 transition-all" disabled={isSubmitting}>Back</button>
