                        ...
                        <div className="p-4">
                            <div className="flex justify-between items-center mb-4">
                                <div>
                                    ...
                                    <p className="text-xs text-slate-500 dark:text-zinc-400">Choose a contact to add or manage their properties.</p>
                                </div>
                                <button 
                                    onClick={() => {
                                    ...
                                    className="px-3 py-1.5 bg-primary-600 hover:bg-primary-700 text-white text-[10px] font-black uppercase tracking-widest rounded-lg shadow-lg shadow-primary-500/20 transition-all flex items-center gap-1.5"
                                >
                            ...
                            <div className="space-y-1.5 max-h-[65vh] overflow-y-auto custom-scrollbar pr-1">
                                {matterState.contacts.length > 0 ? matterState.contacts.sort((a, b) => a.name.localeCompare(b.name)).map(c => (
                                    <button
                                        ...
                                        className="w-full text-left p-3 rounded-xl border border-slate-200 dark:border-zinc-700 hover:border-primary-500 dark:hover:border-primary-500 hover:bg-primary-50/50 dark:hover:bg-primary-900/10 transition-all flex items-center justify-between group"
                                    >
                                        <div className="flex items-center gap-2.5">
                                            <div className="w-8 h-8 rounded-full bg-slate-100 dark:bg-zinc-800 flex items-center justify-center text-slate-500 font-bold text-xs group-hover:bg-primary-100 dark:group-hover:bg-primary-900/30 group-hover:text-primary-600">
                                                {c.name.charAt(0)}
                                            </div>
                                            <div>
                                                <p className="font-bold text-sm text-slate-700 dark:text-zinc-200 group-hover:text-primary-700 dark:group-hover:text-primary-300">{c.name}</p>
                                                <p className="text-[10px] text-slate-500 dark:text-zinc-500">{c.category} &bull; {c.email || 'No email'}</p>
                                            </div>
                                        </div>
                                    ...
                                    <div className="text-center py-12 bg-slate-50 dark:bg-zinc-900/50 rounded-xl border-2 border-dashed border-slate-200 dark:border-zinc-800">
                                        <p className="text-slate-500 dark:text-zinc-400 font-medium text-sm mb-4">No contacts found.</p>
                                        <button 
                                            onClick={() => {
                                                closeModal();
                                                openModal('newContact', null, { returnTo: 'newProperty', openedByAloa: true });
                                            }}
                                            className="px-6 py-2 bg-primary-600 hover:bg-primary-700 text-white text-[10px] font-black uppercase tracking-widest rounded-lg shadow-lg shadow-primary-500/30 transition-all inline-flex items-center gap-2"
                                        >
                                            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="3" d="M12 4v16m8-8H4" /></svg>
