    ...
    return (
        <div className="space-y-6">
            ...
            <div className="text-center">
                ...
                <p className="text-sm text-slate-500 dark:text-zinc-400">Upgrade to {planConfig.title} for full access.</p>
            </div>
                ...
                <ul className="space-y-3">
                    {planConfig.features.map((feat, i) => (
                        <li key={i} className="flex items-start gap-3">
                            <div className="p-1 bg-emerald-100 dark:bg-emerald-900/30 rounded-full text-emerald-600 mt-0.5"><CheckIcon className="w-3 h-3" /></div>
                            <p className="text-sm font-semibold text-slate-700 dark:text-zinc-200">{feat}</p>
                        </li>
                    ))}
                </ul>
            </div>
            ...
            {/* Price + CTA */}
            <div className="p-5 bg-slate-50 dark:bg-zinc-800/50 rounded-xl border border-slate-200 dark:border-zinc-700 text-center">
            ...
            <div className="text-center">
                <button onClick={onClose} className="text-xs text-slate-500 hover:text-slate-800 dark:hover:text-zinc-300 font-medium">
                    Maybe Later
                </button>
            </div>
        </div>
