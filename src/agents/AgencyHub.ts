...
`;
    ...
    // ICM: ALOA identity is sourced from ai/prompts/01-aloa-legal-identity.md
    // via Vite ?raw import. Edit the markdown file to change the identity.
    const aloaIdentity = renderAloaIdentity({
    ...
    return universalContext + aloaIdentity + getInteractiveFormDelegationProtocol(false);
};
