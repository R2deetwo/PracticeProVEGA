...
export const CoreProvider: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
    ...
    // PRACTICE-PROFILE ENGINE — "DB-first" category resolution.
    // Previously these product-mode lists REPLACED whatever the firm had
    // saved, so a firm that configured curated categories (via the Practice
    // Blueprint in onboarding or Settings) never saw its own rows — the
    // hardcoded "stock" list always won. Now:
    //   - firm has rows saved  → show THE FIRM's rows (blueprint-curated)
    //   - firm has no rows yet → fall back to the product-mode stock list
    // The stock lists double as the fallback for firms that skip the
    // blueprint, and the backend seeds product-appropriate rows at
    // createFirm time (see convex/myFunctions.ts createFirm).
    const productCategories = React.useMemo(() => {
        const dbContacts = appState.contactCategories || [];
        const dbDocs = appState.documentCategories || [];
        const categories = {
            contactCategories: dbContacts,
            documentCategories: dbDocs,
        };

        const emptyContacts = dbContacts.length === 0;
        const emptyDocs = dbDocs.length === 0;
