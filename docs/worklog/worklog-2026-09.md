...
Task ID: 47 (backfill)
Agent: Main agent (Super Z)
Task: Task 45/5 backlog #1 — Practice Blueprint re-entry (entry points + merge/preview verification). The implementation commit 84cafa96 shipped 2026-09-16 but its worklog entry was lost when the session ran out of context; this entry backfills the record.

Work Log:
- ENTRY POINTS shipped in 84cafa96: HelpOnboardingPanel "Re-run Practice/Portfolio Blueprint" row (product-aware label, deep-links to the practice-blueprint settings target, resets the processed-target guard first so repeat clicks always re-open) + HelpView admin-guide bullet. These join the three pre-existing entries: the Firm Configuration blueprint card ("Adjust / Re-run" once applied), the Getting Started checklist item, and CompleteSetupBanner.
- VERIFIED COMPLETE (this session, pre-Task-48 code review): the re-entry flow was already safe end-to-end — src/hooks/usePracticeProfile.ts implements the four merge contracts (ADDITIVE ONLY, IDEMPOTENT by case-insensitive name/type match, PREVIEWABLE pure-data plan, workflow MERGE that only adds missing sub-categories to an existing matter type); PracticeProfileSetup.tsx renders the select→preview(→result) wizard with new-vs-existing diff chips, per-group counts, the workflow-merges section and the "Everything already set up" terminal state; TemplatesSettings persists practiceProfile + blueprintAppliedAt on apply; OnboardingWizard applies the same engine at wizard completion.
- TEST GAP CLOSED (in Task 48's commit): buildLegalPlan/buildAtriumPlan exported for tests (same exported-pure-helpers pattern as automationEngine); tests/unit/automationRules.test.ts PART 3 pins the idempotent re-run, the existing-workflow merge (missing sub-categories only), and mergePlans cross-product dedup — the exact guarantees the "Existing data is kept" UI copy promises.

Stage Summary:
- Blueprint re-entry is DONE and test-pinned: a firm that expands into a new practice area can re-open the blueprint from Settings → Help, Firm Configuration, the checklist or the setup banner; re-running only adds what's missing; nothing existing is ever deleted or overwritten.

---
Task ID: 48
Agent: Main agent (Super Z)
Task: Task 45/5 backlog #2b — Automation rule-execution engine (+ blueprint recipe wiring). The automationRules table ("Your Rules", Settings → Firm Configuration → Automations) was UI-only: rules saved but never executed.

Work Log:
- NEW convex/automationRules.ts (~560 lines): pure helpers (ruleTriggerMatches with per-trigger discriminator semantics — exact stage for matter_stage_change, substring for event_created, exact priority for task_overdue, exact source for document_uploaded; ruleDedupKey; ruleMergeVars; buildTaskPayload; buildMessagePayload; isInvoiceOverdue/isTaskOverdue/isOnboardingIncomplete sweep predicates) all exported for tests.
- DISPATCHER dispatchRulesForEvent (plain async fn so myFunctions can call it in-transaction): loads the firm's enabled matching rules, enforces the ONCE-PER-RULE-PER-ENTITY ledger (automation_dispatch_log, workflowKey="rule:<id>", periodKey="once" — reuse of the Atrium engine's table + idempotency contract), then executes actions: create_task → direct tasks insert (matter's assignedUsers, firm-admin fallback, creatorId "automation", idempotencyKey belt-and-braces); send_email / send_whatsapp → automation_logs + scheduled_messages rows (channel-resolved recipient, rendered merge fields, next-tick scheduledFor) so the 5-minute dispatcher delivers with branding + unsubscribe + opt-out enforcement for free; generate_document → honest "skipped_unsupported" ledger outcome (never a silent no-op).
- EVENT HOOKS: createItem dispatches matter_created / lead_created / event_created / document_uploaded; updateItem captures the pre-patch stage and dispatches matter_stage_change ONLY on real changes (both stages non-empty + different); portals.ts sendPortalMessage participant attachments dispatch document_uploaded with value "client_portal" (that IS the client-upload surface; admin-side attachments do not fire). Every hook is try/catch-wrapped — an engine failure can never roll back or fail the user's save.
- CRON ruleEngineSweep daily 6:45 UTC (a quarter-hour after the Atrium engine's 6:30 — never the same write instant): invoice_overdue (open statuses past due; once per invoice forever, never daily nagging), task_overdue (priority-matched, automation-created tasks excluded so a rule can never react to its own output), client_onboarding_incomplete (matters 24h–14d old with zero tasks/documents/events — the honest "nobody worked on it" definition; older dormant matters stay with the anomaly detector).
- SCOPE GUARD: the four property_* triggers are deliberately NOT executed here (owned by the Atrium automationEngine per the single-orchestration-point directive); isExecutableTrigger enforces it.
- getRuleActivity (public query, requireStaffCaller-guarded): per-rule lastFiredAt/total/lastOutcome from the ledger.
- UI (AutomationSettings.tsx): rules list extracted into RulesListWithActivity (own getRuleActivity useQuery behind a SectionErrorBoundary — the 2026-09-14 version-skew isolation pattern); each rule card now shows "fired N times · last <date> · <outcome>"; header copy states rules run automatically + once-per-record; NEW "Ideas from your Practice Blueprint" panel (blueprint recipe wiring) — suggestions drawn from the firm's actual practiceProfile areas/portfolio via getProfilesForAreas + getAtriumProfilesForPortfolio, de-duplicated, capped at 8.
- Generated types: hand-extended convex/_generated/api.d.ts with the automationRules module (codegen needs a deployment link locally; CI regenerates on deploy — same approach as the automationEngine and refunds rounds).
- Tests: tests/unit/automationRules.test.ts — 28 tests in three parts: pure engine logic (trigger discriminators incl. any/empty value, dedup-key uniqueness matrix, merge-var honesty, payload rendering + priority clamp + unknown-tag stripping, sweep predicate edges), source-scanned wiring contracts (all five hooks present + non-blocking, real-stage-change guard, portal participant path, cron 6:45, ledger contract, scheduled_messages origin stamping, task idempotencyKey, honest generate_document, self-output exclusion, staff-guarded query, api.d.ts declaration, UI wiring), and the Task-47 blueprint merge-engine contracts (fresh-workspace additions, idempotent re-run, existing-workflow sub-category merge, mergePlans dedup).
- Gates: convex tsc 0 errors; root tsc 126 errors = exact pre-existing baseline (gate 131; grep "error TS" count, CI method); vitest 869/869 (841 + 28 new); identity audit PASSED; design-token gate gray=0; vite production build ✓.

Stage Summary:
- Automation Studio rules now EXECUTE. Event triggers fire in the same transaction as the user's save (failure-isolated); time triggers sweep daily at 6:45 UTC; the once-per-rule-per-entity ledger makes double-firing structurally impossible; message actions inherit the Scheduled Messages pipeline's dedup, branding, unsubscribe and opt-out enforcement.
- Blueprint re-entry (#1) and the rule engine (#2b) — the two approved Task-45/5 items — are now both code-complete, tested and shipping together.
- Deferred backlog items #3–5 (template seeding, state-specific library variants, others) remain queued.
- SECURITY: the PAT pasted this session is embedded in the sandbox clone's remote URL; the user should revoke it (GitHub → Settings → Developer settings → Personal access tokens) once this round is deployed.

---
Task ID: 49
Agent: Main agent (Super Z)
Task: Task 45/5 backlog #3-5 — document-template seeding, state-specific library variants, AI sub-category feed — plus the user's signup-UX feedback: move "How should you appear in correspondence?" from the signup form to the onboarding stage.

Work Log:
- Sandbox had reset again (244 commits stale, node_modules gone): hard reset to origin/main debc7695 (v1.0.621, build-1004) + npm ci; all prior work (Task 47 blueprint re-entry 84cafa96, Task 48 rule engine 041efdfe) verified already on main.
- UX FIX (user feedback, signup walkthrough): the "How should you appear in correspondence?" card (professional title + legal form) rendered at the signup name/email stage — wrong moment, since that's account creation, not practice description. REMOVED from Signup.tsx (state, card, savePendingIdentity call); ADDED to OnboardingWizard Step 1 create-mode directly under the firm name (same inputs, wizard styling, live example line now interpolates the typed firm name); parked via savePendingIdentity in handleCompleteWizard's finally — the proven App.tsx one-shot applier (firm legal form + user title, header/emails/receipts) needed zero changes.
- #3 TEMPLATE SEEDING: NEW src/config/documentTemplateLibrary.ts — 20 curated Nigerian starter templates: 15 legal (Civil Litigation x3, Corporate x2, Real Estate x2, Family x2, Employment x2, Criminal/Banking/IP/Tax/Oil&Gas/Maritime/TechData/General x2/Immigration x1... = per-area curation incl. Board Resolution, Engagement Letters, Tenancy Agreement, Deed of Assignment, Bail Application affidavit, Witness Statement on Oath, Cease & Desist, NDPA notice, Power of Attorney, FIRS audit response etc.) + 7 Atrium property templates (residential tenancy/rent demand/quit notice, commercial tenancy/service-charge demand, short-let agreement, offer of sale). All content uses ONLY placeholderRegistry vocabulary ([CLIENT NAME], [SUIT NUMBER], [TODAY'S DATE]...) so DraftPro's fill modal + matter/firm auto-fill work out of the box; placeholders auto-extracted at render.
- ENGINE: usePracticeProfile extended — PlanItem.table += documentTemplates/documentTemplateCategories; deps += documentTemplates/documentTemplateCategories/stateKey; counts += documentTemplates; appendTemplateItems() shared helper (categories ensured-once by name against existing rows AND in-plan rows, then templates idempotent by name); APPLY_STAGES += 'Template categories' + 'Document templates' (categories BEFORE templates); applyPlan captures addItem-returned category ids and stamps categoryId onto template rows (existing categories resolved first, same-run creations second) so TemplatesSettings' grouped list renders them correctly; mergePlans dedupes the new tables via the existing (table,label) key.
- #4 STATE-SPECIFIC VARIANTS: template content carries {{STATE_NAME}}/{{HIGH_COURT_CAPTION}}/{{HIGH_COURT_RULES}}/{{STATE_DIVISION}}/{{MAGISTRATE_CAPTION}}/{{FHC_CAPTION}} markers resolved against JURISDICTION_REGISTRY at plan-build time; state baked into the NAME ("Demand Letter (Pre-Action) — Lagos") so a later state change + re-run ADDS the new variant (never overwrites); stateOnly gating (Pre-Action Protocol compliance letter seeds ONLY for Lagos firms — most states have no such protocol).
- UI WIRING: PracticeProfileSetup preview gains the Document Templates group + Templates count chip (grid now sm:grid-cols-7) + result-screen next-step line; wizard passes primaryState as stateKey and shows template counts in the Step-6 review chips + completion toast; TemplatesSettings passes documentTemplates/categories/defaultStateOfPractice; TemplatesSettings document-template list gains an Uncategorized fallback bucket (templates whose categoryId no longer resolves would previously render NOWHERE).
- #5 AI SUB-CATEGORY FEED: DraftProEditor's drafting prompt now prepends the linked matter's blueprint track when the matter's subCategory exists in the firm's workflows — the firm's own stages (track order), standard processes and the matter's current stage, with explicit "user instructions win" precedence; best-effort try/catch, never blocks drafting.
- TESTS: NEW tests/unit/documentTemplateSeeding.test.ts — 14 tests in 3 parts: library content contracts (keys align with profile libraries, well-formedness, 300+ char content, placeholder extraction), state-variant contracts (Lagos vs Delta captions/rules baked in, state-suffixed names, stateOnly gating, zero unresolved {{markers}} across all states), engine contracts (fresh-plan seeding + category ensuring, idempotent re-run, state-change adds-not-overwrites, appendTemplateItems no-double-ensure, mergePlans dedup, atrium seeding). PART 3 of automationRules.test.ts's emptyDeps/afterDeps extended with the new deps fields (would otherwise TypeError on undefined.map).
- GATES: convex tsc 0 errors; root tsc 126 = exact pre-existing baseline (verified by stash-diff: the two DraftProEditor errors pre-exist at shifted line numbers; the one new error was a vitest toBeGreaterThan message arg, fixed); vitest 883/883 (869 + 14 new); identity audit PASSED; design-token gate gray=0; vite build clean 21.7s.

Stage Summary:
- The Task 45/5 backlog is COMPLETE: #1 blueprint re-entry (Task 47), #2b rule engine (Task 48), #3 template seeding, #4 state-specific variants, #5 AI sub-category feed (this task) — all shipped, tested and gate-clean.
