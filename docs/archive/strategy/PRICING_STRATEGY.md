...
## Addendum: Komplet bundle pricing (2026)

**Source of truth:** `src/constants/tiers.ts` (`VEGA_TIERS`, `ATRIUM_TIERS`, `UNIFIED_TIERS`).

- **Atrium (property)** list prices were doubled in 2026 (e.g. Core ₦19k/mo, ₦190k/yr). Limits are operational (units, properties, tenants, WhatsApp notices)—not GB storage.
- **Vega (legal)** storage is explicit per tier: Core 5 GB, Growth 25 GB, Pro 100 GB.
- **Komplet (unified)** is **not** doubled. It is priced as a bundle discount vs buying Vega + Atrium at the same tier:

  `kompletPrice ≈ round((vegaPrice + atriumPrice) × bundleFactor)`

  Bundle factors: Core **0.85**, Growth **0.75**, Pro **0.70** (tune in `KOMPLET_BUNDLE_FACTOR` in `tiers.ts`).

- **Convex limits** for property firms use plan keys `Core`, `Growth`, `Pro` (not `Starter`) in `convex/tierLimits.ts`.
