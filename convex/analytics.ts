...
/**
 * mutation: backfillEvents
 * Internal migration script to generate tracking events for existing data.
 */
export const backfillEvents = mutation({
    args: {},
    handler: async (ctx) => {
        const firms = await ctx.db.query("firms").take(500);
        const users = await ctx.db.query("users").take(500);
        const matters = await ctx.db.query("matters").take(500);

        let count = 0;
        
        // Backfill Firm Creation
        for (const firm of firms) {
            await ctx.db.insert("analytics_events", {
                firmId: firm._id,
                userId: "system",
                event: "Firm Registered",
                properties: { name: firm.name },
                timestamp: firm._creationTime
            });
            count++;
        }

        // Backfill User Onboarding
        for (const user of users) {
             await ctx.db.insert("analytics_events", {
                firmId: user.firmId ?? "none",
        ...
        // Backfill Matter History
        for (const matter of matters) {
             await ctx.db.insert("analytics_events", {
                firmId: matter.firmId ?? "none",

                userId: "system",
                event: "Matter Created",
                properties: { title: matter.title, type: matter.type },
                timestamp: matter._creationTime
            });
            count++;
        }

        return { success: true, eventsCreated: count };
    }
});
