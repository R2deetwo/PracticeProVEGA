...
// This ensures ATRIUM_LIMITS[args.subscriptionPlan] works regardless
// of which label the signup form sends.
const ATRIUM_LIMITS: Record<string, { units: number, tenants: number, whatsapp: number }> = {
...
export const getFirmData = query({
  ...
  handler: async (ctx, args) => {
    ...
    if ((!targetFirmId || targetFirmId === 'skip') && userEmail) {
    ...
    try { 
        ...
        console.log(`[getFirmData] Loading data for firm: ${(firmDetails as any)?.name || targetFirmId} (Email: ${userEmail})`);
    } catch (e) { 
...
export const getFirmMetadata = query({
  ...
  handler: async (ctx, args) => {
    const { firmId } = args;
    if (!firmId || firmId === 'skip') return null;

    const fid = firmId.toString();

    const [matters, contacts, properties, tasks, events, invoices] = await Promise.all([
    ...
    return { matters, contacts, properties, tasks, events, invoices };
  },
});
    ...
    if (isPasswordCorrect) {
      if (user.isMfaEnabled) {
        if (!args.mfaCode) {
          ...
          return { success: false, requiresMfa: true, mfaType: 'email', debugCode: code };
        } else {
...
export const createFirm = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const firmId = await ctx.db.insert("firms", {
      ...
      maxUnits: (args.product === "property" || args.product === "atrium") ? (ATRIUM_LIMITS[args.subscriptionPlan]?.units || 5) : 999999,
      maxTenants: (args.product === "property" || args.product === "atrium") ? (ATRIUM_LIMITS[args.subscriptionPlan]?.tenants || 5) : null,
      whatsappLimit: (args.product === "property" || args.product === "atrium") ? (ATRIUM_LIMITS[args.subscriptionPlan]?.whatsapp || 0) : 0,
...
export const deleteAccount = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const fid of firmsToClean) {
      ...
      try { await ctx.db.delete(fid as any); } catch (e) {}
    }
...
export const deleteFirm = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    try { await ctx.db.delete(args.firmId as any); } catch (e) {}
...
export const createItem = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    const { _id, _creationTime, ...rest } = args.data;

    const sanitizedData: Record<string, any> = {};
    for (const [key, val] of Object.entries(rest)) {
      // 1. Local State Filter: Strip keys starting with _ (except audit trail) or temp
      if (key.startsWith("temp") || (key.startsWith("_") && !["_lastModifiedBy", "_version"].includes(key))) {
        continue;
      }
    ...
    const id = await ctx.db.insert(args.table as any, dataWithTimestamp);
    return id;
  },
});
...
export const updateItem = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const [key, val] of Object.entries(rest)) {
      ...
      patchData[key] = val === null ? undefined : val;
    }

    // Auto-update the modification timestamp and version
    patchData.updatedAt = new Date().toISOString();
    const existing = await ctx.db.get(id as any) as any;
    patchData._version = (existing?._version || 0) + 1;

    await ctx.db.patch(id as any, patchData);
  },
});
...
export const deleteItem = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // 1. Strategy A: Direct Delete (Internal ID)
    try {
  ...
  handler: async (ctx, args) => {
    ...
    for (const table of tablesToClean) {
      try {
        ...
        // Fallback for tables without index or with different field names
        try {
...
export const forceDeleteItem = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    // 1. Try direct delete (Internal ID)
    try {
      await ctx.db.delete(id as any);
      return { success: true, method: "internal_id" };
    } catch (e) {
    ...
    // 2. Search multiple tables for custom 'id' field (UUIDs)
  ...
  handler: async (ctx, args) => {
    ...
    if (matter?.specialtyData?.realEstate?.propertyId) {
      ...
      try {
        ...
        let propRecord = null;
        try { propRecord = await ctx.db.get(propId as any); } catch(e) {}
        if (!propRecord) {
        ...
        console.warn("[Cascade] Linked property delete failed:", propId);
      }
    }
    ...
    for (const table of tablesToClean) {
      try {
        ...
        console.warn(`[Cascade] Failed on table ${table}:`, e);
      }
    }
...
export const deletePropertyCascade = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    try {
        ...
        // Not a valid Convex ID, try lookup by custom id
    }
    ...
    if (!propertyRecord) {
        console.warn(`[Delete Cascade] Property ${propertyId} not found.`);
