...
const CHAKRA_PLANS = {
  free: {
    ...
    recommendedFor: 'Solo landlords with <20 units',
  },
  starter: {
    label: 'Starter',
    whatsappLimit: 5000,
    ...
    recommendedFor: 'Small agencies with 20-50 units',
  },
  pro: {
    label: 'Professional',
    ...
    recommendedFor: 'Established agencies with 50-200 units',
  },
  enterprise: {
    label: 'Enterprise',
    whatsappLimit: 0, // unlimited
    ...
    recommendedFor: 'Large estate operators with 200+ units',
  },
};
...
export function estimateChakraPlan(
  ...
  if (totalUnits <= 20 && estimatedMonthlyMessages <= 800) {
    recommendedPlan = 'free';
    reasoning = `With ${totalUnits} units and ~${estimatedMonthlyMessages} monthly messages, the free plan covers your needs. You'll have ${1000 - estimatedMonthlyMessages} messages headroom.`;
  } else if (totalUnits <= 50 && estimatedMonthlyMessages <= 4000) {
    recommendedPlan = 'starter';
    reasoning = `With ${totalUnits} units and ~${estimatedMonthlyMessages} monthly messages, the Starter plan gives you sufficient capacity with room to grow. Free plan would be too restrictive at this volume.`;
