...
export const AuthProvider: React.FC<{ children?: React.ReactNode }> = ({ children }) => {
    ...
    // OFFLINE MODE STATE
    const [offlineUser, setOfflineUser] = React.useState<any>(null);
    ...
    // 1. Storage is already loaded synchronously, but keep effect for any future side-effects
    React.useEffect(() => {
        setIsStorageLoaded(true);
    }, []);

    // 2. Auto-Heal: If real data arrives while we are in offline mode, switch to live mode automatically
    React.useEffect(() => {
        if (userData && offlineUser) {
    ...
    const currentUser: User | null = React.useMemo(() => {
        ...
        // Use backend data if available, otherwise fallback to offlineUser
        // Priority: Real DB Data > Offline Mock
        const data = userData || offlineUser;

        if (!data) return null;

        // If not verified, effectively not logged in for the app (unless offline mode which implies verified locally)
        if (!data.isVerified && !offlineUser) return null;
        ...
        return combined;
    }, [userData, sessionToken, offlineUser, localUserOverrides]);
    ...
    const login = async (email: string, password?: string, mfaCode?: string, rememberMe: boolean = true) => {
        ...
        try {
            ...
            // 3. Set Session
            setSessionToken(token);
            setOfflineUser(null); // Clear offline state on success
            ...
            // Triggers when: network is down, Convex project is disabled/rate-limited,
            // or any other server infrastructure failure prevents authentication.
            const isInfrastructureFailure =
                errorMsg === "Connection timed out" ||
                errorMsg.includes("Network") ||
                errorMsg.includes("fetch") ||
                errorMsg.includes("CONVEX") ||
                errorMsg.includes("Server Error") ||
                errorMsg.includes("Called by client") ||
                errorMsg.includes("free plan") ||
                errorMsg.includes("disabled") ||
                errorMsg.includes("convex.cloud") ||
                errorMsg.includes("verifyLogin");

            if (isInfrastructureFailure) {
                console.warn("Infrastructure failure detected. Activating Emergency Offline Mode.", errorMsg);
                ...
                setOfflineUser(mockOfflineUser);
                setSessionToken(token);
                sessionStorage.setItem(LOCAL_STORAGE_USER_KEY, JSON.stringify({ token }));

                return { success: true, message: "Server unavailable. Logged in via Offline Mode." };
            }

            // Surface the actual error so the user/developer can see what went wrong
    ...
    const refreshUser = async () => {
        ...
        // 1. Clear offline state to force UI to look for real data
        setOfflineUser(null);
        ...
        try {
            ...
            if (repairResult.success) {
                console.log("✅ Repair Successful! Found and linked firm:", repairResult.firmId);
            ...
            // Re-enable offline mode if the fetch hard fails
            if (!offlineUser && !userData) {
    ...
    React.useEffect(() => {
        if (sessionToken && !userData && !offlineUser && sessionToken !== 'demo@practicepro.ng') {
        ...
        } else if (userData || offlineUser) {
            setHasTimedOut(false);
            setRetryCount(0);
        }
    }, [sessionToken, userData, offlineUser, retryCount]);
    ...
    const isLoading = !isStorageLoaded || (!!sessionToken && userData === undefined && !offlineUser && sessionToken !== 'demo@practicepro.ng' && !hasTimedOut);
    ...
    const value = {
        ...
        updateCurrentUser: (data: Partial<User>) => {
            ...
            if (offlineUser) {
                setOfflineUser((prev: any) => prev ? { ...prev, ...data } : null);
            }
        },
