...
import { getTiersForProduct, ProductMode, TierId, TierDef } from '../../constants/tiers';
...
const OnboardingWizard: React.FC<OnboardingWizardProps> = ({ onComplete }) => {
    ...
    const tierIds: TierId[] = ['Core', 'Growth', 'Pro', 'Enterprise'];
                ...
                {step === 3 && (
                    <div className="space-y-6 max-w-5xl mx-auto">
                        <div className="text-center">
                            ...
                            <p className="text-slate-500 mt-1 text-sm font-medium">All prices are for <span className="font-bold text-slate-700">{product === 'legal' ? 'Vega Legal' : product === 'property' ? 'Atrium Property' : 'Komplet Unified'}</span>.</p>
                        </div>
                        ...
                        {/* Setup fee notice */}
                        {(selectedTierId === 'Enterprise' || isDataMigration) && (
                            <div className="max-w-md mx-auto p-4 rounded-2xl bg-slate-50 border border-slate-100 flex justify-between items-center">
                                <div>
                                    ...
                                    <p className="text-[9px] text-slate-400 font-bold mt-0.5">Required for {selectedTierId === 'Enterprise' ? 'Enterprise Plan' : 'Managed Migration'}</p>
                                </div>
