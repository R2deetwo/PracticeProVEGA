...
const ContactForm: React.FC<ContactFormProps> = ({ onAddContact, onUpdateContact, onClose, contactToEdit, contactCategories, initialContext, isCompact }) => {
    ...
    useEffect(() => {
        if (isEditing && contactToEdit) {
            ...
            setContactType(contactToEdit.contactType);
            const categoryToSet = contactCategories.find(c => c.name === contactToEdit.category);
            setCategory(categoryToSet?.id || '');
            ...
            if (context.name) setName(context.name);
            if (context.email) setEmail(context.email);
            if (context.phone) setPhone(context.phone);
            if (context.address) setAddress(context.address);
            if (context.companyName) {
                setCompanyName(context.companyName);
                setContactType(ContactType.Company);
            }
            if (context.category) {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        if (isEditing && contactToEdit) {
            onUpdateContact({ ...contactToEdit, ...contactData }, false);
        } else {
            onAddContact(contactData, false);
        }
        onClose();
    };
        ...
        <form onSubmit={handleSubmit} className="flex flex-col gap-4 -m-2">
                    ...
                    <div className="flex items-center gap-4 mb-2 px-1">
                        ...
                        {pickerSupport.isSupported && !isEditing && (
                            <button 
                                ...
                                className="ml-auto px-5 py-2.5 bg-gradient-to-r from-emerald-600 via-primary-600 to-indigo-600 text-white text-[9px] font-black uppercase tracking-[0.2em] rounded-xl shadow-xl shadow-primary-500/30 hover:scale-105 active:scale-95 transition-all flex items-center gap-2 border border-white/10 group overflow-hidden relative"
                            >
                            ...
                            <div className="flex justify-between items-center px-1">
                                ...
                                {!isAddingCategory && (
                                    <button 
                                        ...
                                        className="text-[10px] font-black text-primary-600 uppercase tracking-widest hover:underline mb-1.5"
                                    >
                                        + Add New
                                    </button>
                                )}
                            ...
                            {isAddingCategory ? (
                                <div className="flex gap-2">
                                    ...
                                    <button 
                                        ...
                                        className="px-4 py-2 bg-primary-600 text-white rounded-xl text-xs font-bold hover:bg-primary-700 disabled:opacity-50"
                                    >
                                                    ...
                                                    <div key={prop.id} onClick={() => { onClose(); navigateTo('propertyDetail', prop.id); }} className="group p-4 bg-white dark:bg-zinc-900 rounded-2xl border border-slate-100 dark:border-zinc-800 shadow-sm hover:border-primary-300 dark:hover:border-primary-800 transition-all cursor-pointer flex items-center justify-between">
                                                        <div className="flex items-center gap-4">
                                                            ...
                                                            <div>
                                                                ...
                                                                <p className="text-[10px] font-medium text-slate-500 dark:text-zinc-500 uppercase tracking-wider">{prop.category} • {prop.status}</p>
                                                            </div>
                                                        </div>
            ...
            <div className="sticky bottom-0 left-0 right-0 pt-8 bg-white dark:bg-zinc-900 border-t border-slate-100 dark:border-zinc-800 flex flex-wrap-reverse sm:justify-end gap-3 z-20">
                ...
                <button type="submit" className="flex-1 sm:flex-none px-12 py-2.5 bg-primary-600 text-white text-[10px] font-black uppercase tracking-widest rounded-2xl shadow-2xl shadow-primary-500/30 hover:bg-primary-700 hover:scale-[1.02] active:scale-[0.98] transition-all flex items-center justify-center gap-2">
                    <SaveIcon className="w-4 h-4" /> {isEditing ? 'Save Changes' : 'Create Contact'}
                </button>
            </div>
        </form>
    );
