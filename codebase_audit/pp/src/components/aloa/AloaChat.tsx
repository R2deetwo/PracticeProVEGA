...
export const AloaChat: React.FC<{ onClose: () => void; onDraftStream?: (chunk: string) => void; onMinimize?: () => void; isMobile?: boolean }> = ({ onClose, onDraftStream, onMinimize, isMobile }) => {
    ...
    const toggleLiveSession = async () => {
        ...
        const apiKey = isDemo ? 'AIzaSyAjPumBNTGi8Lzg457yKm4dD0jFAzefXo0' : getGeminiApiKey();
        if (!apiKey) {
    ...
    const handleSend = async () => {
        ...
        try {
            ...
            const aloaXLibrary = loadAloaXLibrary();
            ...
            if (currentResponse.text && currentResponse.text.trim()) {
                // Apply identity guardrail validator — catches any generic Gemini leakage
    ...
    return (
        <div className="flex flex-col h-full bg-white dark:bg-zinc-950 overflow-hidden shadow-2xl relative">
            <header className="flex-shrink-0 px-6 py-5 border-b border-slate-200 dark:border-zinc-800 flex justify-between items-center bg-slate-50/50 dark:bg-zinc-900/50">
                <div className="flex items-center gap-2 sm:gap-4 min-w-0 flex-shrink-0">
                    {(activeView === 'quickNote' || activeView === 'form' || activeView === 'details') && (
                        <button 
                            ...
                            className="p-1.5 -ml-1 text-slate-400 hover:text-slate-900 dark:hover:text-white rounded-lg transition-colors bg-white dark:bg-zinc-800 shadow-sm border border-slate-100 dark:border-zinc-700 mr-2"
                        >
                    ...
                    <div className="min-w-0">
                        <div className="flex items-center gap-2">
                            <h2 className="font-extrabold text-base sm:text-lg text-slate-900 dark:text-white leading-none truncate">
                                {activeView === 'form' ? 'Edit Note' : activeView === 'details' ? 'Note Details' : activeView === 'quickNote' ? 'New Note' : (isProperty ? 'ARIA' : 'ALOA')}
                            </h2>
                            {activeView === 'chat' && <ModelBadge model={preferredModel} onClick={cycleModel} />}
                        </div>
                        {activeView === 'chat' && (
                            <p className="text-[10px] text-slate-400 dark:text-zinc-500 font-medium leading-none mt-0.5">
                ...
                <div className="flex items-center gap-1 sm:gap-3 ml-4">
                    ...
                    <button
                        onClick={onClose}
                        className="p-2 text-slate-500 hover:text-red-600 transition-all font-bold bg-slate-100 dark:bg-zinc-800 rounded-lg active:bg-slate-200 dark:active:bg-zinc-700 flex items-center gap-1 border border-slate-200 dark:border-zinc-700"
                ...
                >
                    ...
                    {isLoading && (
                        <div className="flex justify-start animate-pulse">
                            <div className="bg-slate-50 dark:bg-zinc-900 p-4 rounded-2xl rounded-tl-none border border-slate-100 dark:border-zinc-800 flex gap-2 items-center">
