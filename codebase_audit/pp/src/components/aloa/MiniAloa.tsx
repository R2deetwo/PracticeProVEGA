    ...
    const handleSend = async () => {
        ...
        try {
            ...
            while (currentResponse.toolCalls && currentResponse.toolCalls.length > 0 && iterations < 3) {
                ...
                currentResponse = await aiService.sendMessage(history, { appState: { ...coreState, ...matterState, ...executionState, ...financeState, ...documentState } as any, currentUser: currentUser!, currentHistoryEntry, localFiles, aloaXLibrary, isFirmSearchEnabled, searchBrain }, preferredModel);
            }

            if (currentResponse.text) {
                const modelResp: AloaMessage = {
                    id: uuidv4(),
                    role: 'model' as const,
                    content: currentResponse.text!,
                    modelUsed: currentResponse.modelUsed
                };
                setMessages(prev => [...prev, modelResp]);
            }

        } catch (error: any) {
            console.error('MiniAloa Error:', error);
            const errorMsg: AloaMessage = {
                id: uuidv4(),
                role: 'model' as const,
            ...
            <div className="flex-1 overflow-y-auto p-3 space-y-4 bg-transparent custom-scrollbar">
                        ...
                        <div className={`max-w-[90%] p-3 rounded-xl text-[12px] leading-relaxed shadow-sm relative ${msg.role === 'user'
                            ...
                            {msg.role === 'model' && !msg.isError && (
                                <button
                                    ...
                                    className="absolute -right-2 -bottom-2 p-1 bg-white dark:bg-zinc-700 border border-slate-200 dark:border-zinc-600 rounded-md text-slate-400 hover:text-emerald-500 shadow-md opacity-0 group-hover/msg:opacity-100 transition-all active:scale-90 z-20"
                                    title="Save to Notes"
                                >
                ...
                {isLoading && (
                    <div className="flex justify-start">
                        <div className="bg-white/50 dark:bg-zinc-800/50 backdrop-blur-sm p-3 rounded-xl rounded-tl-none flex items-center gap-1.5 shadow-sm border border-white/20">
