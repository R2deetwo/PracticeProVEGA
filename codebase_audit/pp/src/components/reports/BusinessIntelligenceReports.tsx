...
const BusinessIntelligenceReports: React.FC<BusinessIntelligenceReportsProps> = () => {
    const [activeTab, setActiveTab] = useState<BITab>('case');
        ...
        <div>
            <div className="mb-6 border-b border-gray-200 dark:border-zinc-700">
                <nav className="-mb-px flex space-x-6 overflow-x-auto">
                    {(['case', 'client', 'property'] as BITab[]).map(tab => {
                        const label = { case: 'Case Analytics', client: 'Client Analytics', property: 'Property Analytics' }[tab];
                        return (
                             <button
                                key={tab}
