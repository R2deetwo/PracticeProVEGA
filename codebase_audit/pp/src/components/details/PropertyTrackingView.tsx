...
const PropertyTrackingView: React.FC<PropertyTrackingViewProps> = ({ property, onUpdate }) => {
    ...
    const handleDownloadReceipt = (payment: RentPayment) => {
        try {
            const mockInvoice: any = {
                id: uuidv4(),
                invoiceNumber: payment.receiptNumber || `REC-${Math.floor(100000 + Math.random() * 900000)}`,
                client: { id: 'tenant', name: property.rentalDetails?.tenantName || 'The Tenant' },
            ...
            generateReceiptPdf(
                ...
                { name: property.rentalDetails?.tenantName || 'The Tenant' } as any,
                { tenancyPeriod }
            );
...
export default PropertyTrackingView;
