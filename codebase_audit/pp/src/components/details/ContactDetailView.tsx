...
type ContactTab = 'overview' | 'matters' | 'properties' | 'relationships';

export const ContactDetailView: React.FC<ContactDetailViewProps> = ({ contactId, onGoBack, openModal, isPane }) => {
  ...
  const handleDeleteProperty = (property: Property) => {
    openModal('deleteConfirmation', property.id, {
        ...
        onConfirm: () => {
            // Check if it's a standalone or legacy property
            const isStandalone = (coreState.properties || []).some(p => p.id === property.id);
            if (isStandalone) {
                dataHandlers.deleteItem('properties', property.id, property.address);
            } else {
    ...
    <div data-item-id={contact.id} ref={containerRef} className="h-full flex flex-col">
      ...
      <div className="flex-shrink-0 px-6 border-b border-slate-200 dark:border-zinc-700 bg-white dark:bg-zinc-800">
            <nav className="-mb-px flex space-x-6">
                <button onClick={() => setActiveTab('overview')} className={`py-3 px-1 border-b-2 font-semibold text-sm transition-colors ${activeTab === 'overview' ? 'border-primary-500 text-primary-600 dark:text-primary-400' : 'border-transparent text-slate-500 hover:text-slate-700 dark:text-zinc-400'}`}>Overview</button>
