...
const FirmSettings: React.FC<FirmSettingsProps> = ({ firmDetails, onUpdateFirmDetails, openModal, users, currentUser, openUserModal, onDeleteUser, onEnableDevMode }) => {
    ...
    const isHighTier = currentPlan === SubscriptionPlan.Pro || currentPlan === SubscriptionPlan.Enterprise || currentPlan === SubscriptionPlan.Komplete;
                ...
                <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
                    <div className="text-sm text-slate-600 dark:text-zinc-400">
                        ...
                        <p className="max-w-md">
                            {isHighTier
                                ? "As a Pro or Enterprise user, you can join multiple workspaces and switch between them instantly while retaining data."
                        ...
                        {!isHighTier && (
                            <p className="mt-2 text-xs text-blue-600 dark:text-blue-400 flex items-center cursor-pointer hover:underline" onClick={() => navigateTo('settings', null, { settingsTargetId: 'subscription-management' })}>
                                <ShieldCheckIcon className="w-3 h-3 mr-1" /> Upgrade to Pro or Enterprise to enable Multi-Workspace Retention.
                            </p>
                        )}
                    </div>
