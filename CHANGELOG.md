...
- **Starter document templates, seeded from your Blueprint** — picking your practice areas (or portfolio composition) now also pre-fills matching document templates: a Civil Litigation firm gets a demand letter, a witness statement on oath and more; a residential portfolio gets a tenancy agreement, rent demand notice and notice to quit. Every template uses fill-in placeholders that auto-fill from the matter and firm. Existing templates are never touched; re-running the Blueprint only adds what's missing. Manage them under Settings → Firm Configuration → Templates.
- **Templates tailored to your state** — state-aware templates (demand letters, court documents, tenancy agreements) are pre-filled with your state's High Court caption and Civil Procedure Rules, and the template name says which state it was drafted for. Lagos firms additionally get the Pre-Action Protocol compliance letter required by the Lagos High Court Rules 2019. Change your primary state later and re-run the Blueprint to get the new state's variants alongside the old ones.
- **AI drafts follow your workflow track** — when you draft for a matter that runs on one of your blueprint sub-category tracks (e.g. "Debt Recovery (Undefended List)"), the drafting AI is told your firm's stages and standard processes for that track, so drafts match your procedure instead of a generic one.
- **"How should you appear in correspondence?" moved to setup** — the optional role + legal-form card no longer interrupts account creation; it now appears in the setup wizard right under your firm name, where the rest of your workspace identity is captured.
